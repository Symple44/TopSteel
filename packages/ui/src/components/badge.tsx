  ...
  'inline-flex items-center rounded-full border font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2',
  {
    variants: {
      variant: {
        ...
        steel: 'border-transparent bg-gradient-to-r from-slate-600 to-slate-700 text-white hover:from-slate-500 hover:to-slate-600',
        iron: 'border-transparent bg-gradient-to-r from-gray-800 to-gray-900 text-white hover:from-gray-700 hover:to-gray-800',
        aluminum: 'border-transparent bg-gradient-to-r from-slate-300 to-slate-400 text-slate-900 hover:from-slate-200 hover:to-slate-300'
      },
      size: {
...
const Badge = React.forwardRef<HTMLDivElement, BadgeProps>(({
  ...
  const finalVariant = variant || getStatusVariant(status) || getPriorityVariant(priority) || 'default'
  ...
  const content = (
    <>
      ...
      {dot && (
        <span 
          className={cn(
            ...
            finalVariant === 'success' ? 'bg-green-400' :
            finalVariant === 'warning' ? 'bg-yellow-400' :
            finalVariant === 'destructive' ? 'bg-red-400' :
            'bg-blue-400'
          )}
        />
      )}
      ...
      {icon && iconPosition === 'start' && !loading && (
        <span className={cn('mr-1', size === 'sm' ? 'w-3 h-3' : 'w-4 h-4')}>
          {icon}
        </span>
      )}
      ...
      {icon && iconPosition === 'end' && !loading && (
        <span className={cn('ml-1', size === 'sm' ? 'w-3 h-3' : 'w-4 h-4')}>
          {icon}
        </span>
      )}
  ...
  const accessibilityProps = {
    ...
    onKeyDown: (clickable || onClick) && !disabled ? (e: React.KeyboardEvent) => {
 ...
 */
export const StatusBadge = React.forwardRef<HTMLDivElement, 
 ...
 */
export const PriorityBadge = React.forwardRef<HTMLDivElement,
...
>(({ priority, icon, ...props }, ref) => {
  ...
  return (
    <Badge 
      ref={ref} 
      priority={priority} 
      icon={icon || priorityIcons[priority]}
      {...props} 
    />
  )
})
 ...
 * Badge de compteur
 */
export const CountBadge = React.forwardRef<HTMLDivElement,
