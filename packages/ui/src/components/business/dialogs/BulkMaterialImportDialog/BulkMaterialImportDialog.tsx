...
export function BulkMaterialImportDialog({
  ...
  const availableFields = useMemo(() => [
    ...
    { key: 'tensileStrength', label: 'Résistance traction (MPa)', required: false, dataType: 'number' as const },
    { key: 'yieldStrength', label: 'Limite élasticité (MPa)', required: false, dataType: 'number' as const },
    ...
    { key: 'corrosionResistance', label: 'Résistance corrosion', required: false, dataType: 'string' as const },
    { key: 'isActive', label: 'Actif', required: false, dataType: 'boolean' as const },
    { key: 'requiresInspection', label: 'Contrôle qualité', required: false, dataType: 'boolean' as const },
    ...
    reader.onload = (e) => {
      try {
        ...
        const initialMapping: ImportColumn[] = headers.map(header => {
          ...
          const suggestedField = availableFields.find(field => 
  ...
  const validateMapping = () => {
    const mappedFields = columnMapping.filter(col => col.targetField).map(col => col.targetField)
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-center space-x-4 py-4 border-b">
          <div className={`flex items-center gap-2 ${step === 'upload' ? 'text-blue-600 font-medium' : step === 'mapping' || step === 'preview' || step === 'result' ? 'text-green-600' : 'text-gray-400'}`}>
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${step === 'upload' ? 'bg-blue-600 text-white' : step === 'mapping' || step === 'preview' || step === 'result' ? 'bg-green-600 text-white' : 'bg-gray-200'}`}>1</div>
            Fichier
          </div>
          <div className="w-8 h-px bg-gray-300" />
          <div className={`flex items-center gap-2 ${step === 'mapping' ? 'text-blue-600 font-medium' : step === 'preview' || step === 'result' ? 'text-green-600' : 'text-gray-400'}`}>
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${step === 'mapping' ? 'bg-blue-600 text-white' : step === 'preview' || step === 'result' ? 'bg-green-600 text-white' : 'bg-gray-200'}`}>2</div>
            Mapping
          </div>
          <div className="w-8 h-px bg-gray-300" />
          <div className={`flex items-center gap-2 ${step === 'preview' ? 'text-blue-600 font-medium' : step === 'result' ? 'text-green-600' : 'text-gray-400'}`}>
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${step === 'preview' ? 'bg-blue-600 text-white' : step === 'result' ? 'bg-green-600 text-white' : 'bg-gray-200'}`}>3</div>
            Aperçu
          </div>
          <div className="w-8 h-px bg-gray-300" />
          <div className={`flex items-center gap-2 ${step === 'result' ? 'text-blue-600 font-medium' : 'text-gray-400'}`}>
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${step === 'result' ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}>4</div>
            Résultat
          </div>
        </div>
        {error && (
            ...
            {file && (
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="flex items-center space-x-2 mb-3">
                  ...
                  />
                  <Label htmlFor="skipFirst" className="text-sm">
        ...
        {step === 'mapping' && (
          <div className="flex-1 overflow-y-auto space-y-4">
            <div className="text-sm text-gray-600 mb-4">
              Associez les colonnes de votre fichier aux champs du système. Les champs marqués d'un * sont obligatoires.
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-sm">
                ...
                <Badge variant="outline">
                  {csvData.length} lignes au total
                </Badge>
              </div>
            </div>
