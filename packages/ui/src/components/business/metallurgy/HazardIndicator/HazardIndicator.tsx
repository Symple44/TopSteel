...
import { AlertTriangle, Flame, Skull, Zap, Eye, Thermometer, Wind, ShieldAlert } from 'lucide-react'
interface Hazard {
  id: string
  type: 'fire' | 'toxic' | 'corrosive' | 'explosive' | 'electrical' | 'radiation' | 'biological' | 'environmental'
...
export function HazardIndicator({ 
  ...
  const getSeverityBadge = (severity: Hazard['severity']) => {
    switch (severity) {
      case 'low':
        return <Badge variant="outline" className="text-xs border-yellow-200 text-yellow-600">Low Risk</Badge>
      case 'medium':
        return <Badge variant="outline" className="text-xs border-orange-200 text-orange-600">Medium Risk</Badge>
      case 'high':
        return <Badge variant="destructive" className="text-xs">High Risk</Badge>
      case 'critical':
        return <Badge variant="destructive" className="text-xs bg-red-600">Critical Risk</Badge>
      default:
        return <Badge variant="outline" className="text-xs">Unknown</Badge>
    }
  }
  const getHazardTypeLabel = (type: Hazard['type']) => {
    switch (type) {
      case 'fire': return 'Flammable'
      case 'toxic': return 'Toxic'
      case 'corrosive': return 'Corrosive'
      case 'explosive': return 'Explosive'
      case 'electrical': return 'Electrical'
      case 'radiation': return 'Radioactive'
      ...
      <div className={cn("flex items-center gap-2", className)}>
        {hazards.length === 0 ? (
          ...
          <>
            ...
            {getSeverityBadge(hazards.reduce((max, h) => 
              ['low', 'medium', 'high', 'critical'].indexOf(h.severity) > 
              ['low', 'medium', 'high', 'critical'].indexOf(max) ? h.severity : max, 'low'
            ))}
          </>
        )}
      </div>
