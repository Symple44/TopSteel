...
import { Calculator, Ruler, Weight, RotateCcw, AlertCircle, ArrowLeftRight, ArrowUpDown } from 'lucide-react'
...
export function DimensionInput({
  ...
  }, [dimensions, showCalculations, materialType, materialDensity])
  ...
  const performCalculations = useCallback(async () => {
    ...
    try {
      if (onCalculate) {
        ...
        const calcs = calculateDimensions(dimensions, currentPreset, materialType, materialDensities)
  ...
  }, [dimensions, currentPreset, materialType, onCalculate])
  ...
  const handleDimensionChange = (field: keyof DimensionValue, newValue: string) => {
    ...
    if (minValues?.[field] && numericValue < minValues[field]!) return
    if (maxValues?.[field] && numericValue > maxValues[field]!) return
  ...
  const handlePresetChange = (newPreset: DimensionPreset) => {
    ...
    let resetDimensions = { ...dimensions }
    ...
    <div className={cn('space-y-4', className)}>
      ...
      <div className="grid gap-4">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {(currentPreset === 'rectangular' || currentPreset === 'square' || currentPreset === 'custom') && (
            <div>
          ...
          {currentPreset === 'cylindrical' && (
            <div>
              {renderDimensionField('diameter', <Ruler className="h-4 w-4" />, 'Diamètre')}
            </div>
          )}
          {(currentPreset === 'rectangular' || currentPreset === 'cylindrical' || currentPreset === 'square' || currentPreset === 'custom') && (
            <div>
          ...
          {currentPreset === 'custom' && (
            <>
              ...
              <div>
                {renderDimensionField('weight', <Weight className="h-4 w-4" />, 'Poids', 'weightUnit')}
              </div>
            </>
          )}
        </div>
            ...
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
              {calculations.volume && (
                <div className="text-sm">
                  ...
                  <div className="font-mono font-medium">
                    {calculations.volume.toFixed(6)} m³
                  </div>
                </div>
              )}
              ...
              {calculations.perimeter && (
                <div className="text-sm">
                  ...
                  <div className="font-mono font-medium">
                    {calculations.perimeter.toFixed(4)} m
                  </div>
                </div>
              )}
        ...
        {(dimensions.length || dimensions.width || dimensions.height || dimensions.diameter) && (
          <div className="flex flex-wrap gap-2">
            ...
            {dimensions.length && (
              <Badge variant="outline" className="font-mono">
                L: {formatNumber(dimensions.length)}{dimensions.unit}
              </Badge>
            )}
            {dimensions.width && (
              <Badge variant="outline" className="font-mono">
                l: {formatNumber(dimensions.width)}{dimensions.unit}
              </Badge>
            )}
            {dimensions.height && (
              <Badge variant="outline" className="font-mono">
                H: {formatNumber(dimensions.height)}{dimensions.unit}
              </Badge>
            )}
            {dimensions.diameter && (
              <Badge variant="outline" className="font-mono">
                Ø: {formatNumber(dimensions.diameter)}{dimensions.unit}
              </Badge>
            )}
            {dimensions.weight && (
              <Badge variant="outline" className="font-mono">
                Poids: {formatNumber(dimensions.weight)}{dimensions.weightUnit}
              </Badge>
            )}
          </div>
        )}
