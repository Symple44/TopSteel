...
const TaskCard = ({ task, onTaskClick, onTaskEdit, isTaskOverdue }: TaskCardProps) => {
  const StatusIcon = statusConfig[task.status].icon
  const _PriorityIcon = priorityConfig[task.priority].icon
  ...
  return (
    <button
      className={cn(
        'p-3 bg-white border rounded-lg shadow-sm cursor-pointer transition-all hover:shadow-md text-left w-full',
    ...
    >
      ...
      {task.estimatedHours && (
        <div className="flex items-center justify-between mt-2 text-xs text-muted-foreground">
          ...
          {task.loggedHours && <div>Réalisé: {task.loggedHours}h</div>}
        </div>
      )}
...
export function TaskWorkflow({
  ...
  const isTaskOverdue = (task: TaskItem) => {
    return Boolean(task.dueDate && new Date() > task.dueDate && task.status !== 'done')
  }
  if (view === 'list') {
    return (
          ...
          <div className="flex items-center justify-between">
            ...
            {showMetrics && (
              <div className="flex items-center gap-4 text-sm">
                <span>
                  {metrics.completed}/{metrics.total} terminées
                </span>
                <Badge variant="outline">{Math.round(progressPercentage)}%</Badge>
              </div>
            )}
          </div>
          {showMetrics && <Progress value={progressPercentage} className="h-2" />}
        </CardHeader>
        <CardContent>
