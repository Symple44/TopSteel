    ...
    rules.forEach((rule) => {
      ...
      switch (groupBy) {
        case 'article':
          ...
          _label = rule.articleFamily || rule.articleId || 'Général'
          break
        case 'customer':
          key = rule.customerGroup || 'all'
          _label = rule.customerGroup || 'Tous clients'
          break
        case 'channel':
          key = rule.channel
            ...
            <>
              {groupBy !== 'none' && (
                <TableRow key={`group-${group.groupKey}`}>
                  <TableCell colSpan={editable ? 10 : 9} className="bg-gray-50 font-medium">
  ...
  const renderGridView = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {rules.map((rule) => (
        <Card
          key={rule.id}
          className={cn('relative', selectedRules.has(rule.id) && 'ring-2 ring-primary')}
        >
          {editable && (
