...
export class DragDropUtils {
  ...
  static handleColumnDrop<T>(
    ...
    try {
      ...
      const { newColumns, newSettings } = DragDropUtils.reorderColumns(
        columns,
        settings,
        sourceColumnIndex,
        targetColumnIndex
...
export const useDragDropColumns = <T>(
  ...
  const handleReorderColumns = (newColumns: ColumnConfig<T>[], newSettings: TableSettings) => {
    onColumnsChange(newColumns)
    onSettingsChange(newSettings)
  }

  const handleColumnDragStart =
    (columnId: string, columnIndex: number) => (e: React.DragEvent<HTMLElement>) => {
