...
export interface UseDataExportReturn {
  ...
  canExport: boolean
  prepareExportData: (options?: ExportOptions) => any[]
...
export function useDataExport<T extends Record<string, any>>({
  ...
  const formatValue = useCallback((value: any, column: ColumnConfig<T>): any => {
  ...
  const prepareExportData = useCallback((options: ExportOptions = {}): any[] => {
