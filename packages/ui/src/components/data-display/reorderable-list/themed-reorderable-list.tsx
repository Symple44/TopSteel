...
function useDragAndDrop<T extends ReorderableItem>(
  _items: T[],
  _onItemsChange?: (items: T[]) => void,
  _onExpandItem?: (id: string) => void
) {
...
function createThemeStyles(theme: ReorderableTheme) {
  return {
    ...
    item: (isHover: boolean, isDragging: boolean, isActive: boolean) => ({
      ...
      color: isActive
        ? theme.item.textColorActive
        : isHover
          ? theme.item.textColorHover
          : theme.item.textColor,
      boxShadow: isHover ? theme.item.shadowHover : theme.item.shadow,
      transition: theme.item.transition,
      transform:
        theme.animations.hover && isHover && !isDragging
    ...
    dragHandle: (isHover: boolean, isActive: boolean) => ({
      ...
      backgroundColor: isActive
        ? theme.dragHandle.backgroundActive
        : isHover
          ? theme.dragHandle.backgroundHover
          : theme.dragHandle.background,
      border: `1px solid ${theme.dragHandle.border}`,
      color: isActive
        ? theme.dragHandle.iconColorActive
        : isHover
          ? theme.dragHandle.iconColorHover
          : theme.dragHandle.iconColor,
    }),
    hierarchy: {
    ...
    return (
      <div
        ...
        style={{
          ...
          animation:
            theme.dropIndicator.inside.animation === 'pulse'
              ? 'pulse 1s infinite'
              : theme.dropIndicator.inside.animation === 'bounce'
                ? 'bounce 1s infinite'
                : 'none',
        }}
      >
        <div className="absolute top-1 right-1 text-sm">{theme.dropIndicator.inside.icon}</div>
      </div>
    )
  }
...
function ThemedSortableItem<T extends ReorderableItem>({
  ...
  return (
    <div className="relative">
      <ThemedDropIndicator position={dropPosition} show={isDragOver} theme={theme} />
                ...
                <Button
                  ...
                  style={{
                    ...
                    backgroundColor: isExpanded
                      ? theme.expandButton.backgroundActive
                      : isHover
                        ? theme.expandButton.backgroundHover
                        : theme.expandButton.background,
                  }}
                >
                  {isExpanded ? (
                    <ChevronDown
                      className="h-4 w-4 transition-all duration-200"
                      style={{ color: theme.expandButton.iconColorActive }}
                    />
                  ) : (
                    <ChevronRight
                      className="h-4 w-4 transition-all duration-200"
                      style={{ color: theme.expandButton.iconColor }}
                    />
                  )}
                </Button>
              </div>
                    ...
                    {Array.from({ length: level }, (_, i) => (
                      <div
                        ...
                        style={{
                          ...
                          backgroundColor:
                            i === level - 1
                              ? theme.hierarchy.levelIndicator.colorActive
                              : theme.hierarchy.levelIndicator.color,
                        }}
                      />
                    ))}
              ...
              <div
                ...
                style={{
                  ...
                  backgroundColor: isHover
                    ? theme.hierarchy.connectionLineHover
                    : theme.hierarchy.connectionLine,
                }}
              />
            )}
    ...
    <div className="relative">
      <div className="flex gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="space-y-1 relative" style={containerStyles}>
