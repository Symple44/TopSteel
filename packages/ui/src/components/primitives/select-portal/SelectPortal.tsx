...
export function SelectPortal({
  ...
  const adjustPosition = React.useCallback(() => {
    ...
    const maxX = containerRect
      ? Math.min(viewport.width - 8, containerRect.right - 8)
      : viewport.width - 8
    ...
    const spaceBelow =
      (containerRect ? containerRect.bottom : viewport.height) - triggerRect.bottom - 8
  ...
  React.useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        target.closest('[data-select-portal], [data-category-dropdown], [data-pagesize-dropdown]')
      ) {
        return
      }
