...
export interface DropdownFixedProps {
...
export function DropdownFixed({
  ...
  const calculatePosition = React.useCallback(() => {
    ...
    if (y < 8) y = 8
    if (y + estimatedDropdownHeight > viewport.height)
      y = viewport.height - estimatedDropdownHeight - 8
  ...
  return (
    <>
      <div ref={triggerRef} className={cn('inline-block', className)} {...triggerProps}>
        {children}
      </div>
