...
export default function BackendErrorPage() {
  ...
  const getStatusIcon = () => {
    switch (health.status) {
      ...
      case 'checking':
        return <RefreshCw className="h-8 w-8 text-yellow-600 animate-spin" />
      default:
          ...
          <div className="space-y-3">
            {health.status === 'online' ? (
              ...
              <>
                ...
                <div className="text-sm text-gray-500">{t('backend.autoCheck')}</div>
              </>
            )}
          </div>
            ...
            <details className="mt-6 text-left">
              ...
              <div className="mt-3 text-sm text-gray-600 space-y-2">
                ...
                <p>
                  •{' '}
                  {t('backend.checkPort', {
                    port: process.env.NEXT_PUBLIC_API_URL?.split(':')[2]?.split('/')[0] || '3002',
                  })}
                </p>
