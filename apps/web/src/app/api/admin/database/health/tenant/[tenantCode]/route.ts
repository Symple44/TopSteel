...
export async function GET(
  ...
  try {
    ...
    const apiUrl = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3002'}/api/v1/admin/database/health/tenant/${tenantCode}`
    ...
    const response = await callBackendFromApi(request, `admin/database/health/tenant/${tenantCode}`, {
      method: 'GET',
    ...
    console.error('Erreur lors de la récupération de la santé du tenant:', error)
    return NextResponse.json(
