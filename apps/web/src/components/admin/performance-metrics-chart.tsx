...
export default function PerformanceMetricsChart({ 
  ...
  const [selectedMetric, setSelectedMetric] = useState<'responseTime' | 'queryCount' | 'activeConnections'>('responseTime')
  ...
  const getMetricValue = (point: PerformanceDataPoint) => {
    switch (selectedMetric) {
      case 'responseTime': return point.responseTime
      case 'queryCount': return point.queryCount
      case 'activeConnections': return point.activeConnections
    }
  }
  ...
  const getMetricUnit = () => {
    switch (selectedMetric) {
      ...
      case 'queryCount': return '/min'
      case 'activeConnections': return ''
    }
  }
  ...
  const getMetricLabel = () => {
    switch (selectedMetric) {
      ...
      case 'queryCount': return 'Requêtes'
      case 'activeConnections': return 'Connexions actives'
    }
  }
  ...
  const getMetricIcon = () => {
    switch (selectedMetric) {
      ...
      case 'queryCount': return <Zap className="w-4 h-4" />
      case 'activeConnections': return <Activity className="w-4 h-4" />
    }
  }
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Monitoring en temps réel des performances système
            </CardDescription>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">
              {getCurrentAverage()}{getMetricUnit()}
            </div>
            <div className="text-xs text-muted-foreground">Moyenne</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600">
              {minValue}{getMetricUnit()}
            </div>
            <div className="text-xs text-muted-foreground">Minimum</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-red-600">
              {maxValue}{getMetricUnit()}
            </div>
          ...
          <div className="text-center">
            <div className="flex items-center justify-center space-x-1">
              <TrendingUp className={`w-4 h-4 ${
                getTrend() === 'up' ? 'text-red-500' : 
                getTrend() === 'down' ? 'text-green-500' : 
                'text-gray-500'
              }`} />
            </div>
        ...
        {currentData.length > 0 ? (
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-xs text-muted-foreground">
              <span>
                {new Date(currentData[Math.max(0, currentData.length - 20)].timestamp).toLocaleTimeString()}
              </span>
              <span>
