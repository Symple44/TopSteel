...
import { defaultTheme } from './reorderable-list-themes'
...
export function useReorderableConfig(componentId: string, userId?: string) {
  ...
  const loadConfig = useCallback(async () => {
    try {
      ...
      console.warn('Erreur lors du chargement de la configuration:', err)
  ...
  const saveConfig = useCallback(async (newConfig: Partial<ReorderableListConfig>) => {
    try {
      setSaving(true)
      setError(null)
      ...
      if (response.ok) {
        ...
        localStorage.setItem(
          `reorderable-config-${componentId}`, 
  ...
  const updatePreferences = useCallback(async (preferences: Partial<ReorderableListConfig['preferences']>) => {
    await saveConfig({
      preferences: {
        ...config.preferences,
        ...preferences
  ...
  const updateLayout = useCallback(async (layout: Partial<ReorderableListConfig['layout']>) => {
    await saveConfig({
      layout: {
        ...config.layout,
        ...layout
