...
export class RangeSelectionManager {
   ...
   */
  private isCellInRange(row: number, column: string, range: CellRange, columnOrder?: string[]): boolean {
   ...
   */
  async copyToClipboard<T>(data: T[], columns: Array<{id: string, key: keyof T | string, getValue?: (row: T) => any}>) {
    ...
    const cellsByRow = selectedCells.reduce((acc, cell) => {
      if (!acc[cell.row]) acc[cell.row] = {}
      acc[cell.row][cell.column] = cell
      return acc
    }, {} as Record<number, Record<string, CellPosition>>)

    const rows = Object.keys(cellsByRow).map(k => parseInt(k)).sort((a, b) => a - b)
    ...
    const tsvData = rows.map(rowIndex => {
    ...
    try {
      ...
      console.log('Données copiées:', tsvData)
    } catch (error) {
