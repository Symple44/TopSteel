...
import { ColumnConfig, ExportOptions, ImportResult } from './types'
...
export class ExportUtils {
  ...
  static exportToExcel<T>(
    ...
    try {
      ...
      this.applyAdvancedFormatting(ws, exportColumns, exportData, options)
      ...
      if (options.includeStyles) {
        this.addStatisticsSheet(wb, exportData, exportColumns)
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          // Traiter les données
          const result = this.processImportData(jsonData as any[][], columns)
          resolve(result)
  ...
  private static processImportData<T>(
    ...
    // Mapper les colonnes
    const columnMap = this.mapColumns(headers, columns)
    ...
    dataRows.forEach((row, index) => {
      ...
      columns.forEach(column => {
        ...
        const validation = this.validateAndConvertValue(rawValue, column)
  ...
  private static mapColumns<T>(
    ...
    columns.forEach(column => {
      ...
      let headerIndex = headers.findIndex(header => 
        header?.toLowerCase().trim() === column.title.toLowerCase().trim()
      )
  ...
  private static applyAdvancedFormatting<T>(
    ...
    for (let row = 1; row <= range.e.r; row++) {
      for (let col = 0; col <= range.e.c; col++) {
        ...
        const cellStyle = this.getCellStyle(ws[cellRef].v, column, row)
        ws[cellRef].s = cellStyle
        
        // Format numérique selon le type
        if (column.type === 'number' && column.format) {
          ws[cellRef].z = this.getNumberFormat(column.format)
    ...
    if (options.conditionalFormatting) {
      this.applyConditionalFormatting(ws, columns, range)
    }
  }
      ...
      case 'select':
        if (column.options) {
          ...
          if (option?.color) {
            ...
            const isDark = this.isColorDark(rgbColor)
            if (isDark) {
  ...
  private static addStatisticsSheet<T>(
    ...
    columns.forEach(column => {
      const values = data.map(row => row[column.title]).filter(v => v !== null && v !== undefined && v !== '')
