...
export function useDataViews<T = any>(
  data: T[],
  ...
  const transformToCard = useCallback((item: T, config: any, columns: ColumnConfig<T>[]): Card => {
    ...
    const subtitleColumn = subtitleColumnId ? columns.find(c => c.id === subtitleColumnId) : null
    const descriptionColumn = descriptionColumnId ? columns.find(c => c.id === descriptionColumnId) : null
    const imageColumn = imageColumnId ? columns.find(c => c.id === imageColumnId) : null

    const title = titleColumn 
      ? String((titleColumn.getValue ? titleColumn.getValue(item) : (item as any)[titleColumn.key]) || 'Sans titre')
      : 'Sans titre'

    const subtitle = subtitleColumn 
      ? String((subtitleColumn.getValue ? subtitleColumn.getValue(item) : (item as any)[subtitleColumn.key]) || '')
      : undefined

    const description = descriptionColumn
      ? String((descriptionColumn.getValue ? descriptionColumn.getValue(item) : (item as any)[descriptionColumn.key]) || '')
      : undefined

    const image = imageColumn
      ? String((imageColumn.getValue ? imageColumn.getValue(item) : (item as any)[imageColumn.key]) || '')
      : undefined
      ...
      labelsColumns.forEach((colId: string) => {
        if (colId) {
          ...
          if (column) {
            ...
            if (column.type === 'boolean' || typeof value === 'boolean' || value === true || value === false || value === 'true' || value === 'false') {
              const checkmark = Boolean(value === true || value === 'true') ? '☑' : '☐'
              labels.push({
  ...
  const kanbanData = useMemo(() => {
    ...
    data.forEach(item => {
      const value = statusColumn.getValue ? statusColumn.getValue(item) : (item as any)[statusColumn.key]
    ...
    data.forEach(item => {
      const statusValue = statusColumn.getValue ? statusColumn.getValue(item) : (item as any)[statusColumn.key]
      const normalizedStatus = statusValue === null || statusValue === undefined ? 'Non défini' : String(statusValue)
  ...
  }, [data, columns, viewConfigs.kanban])
  ...
  }, [data, columns, viewConfigs.cards])
  ...
  const timelineData = useMemo(() => {
    ...
    return data.map((item, index) => {
      const dateValue = dateColumn.getValue ? dateColumn.getValue(item) : (item as any)[dateColumn.key]
      const titleValue = titleColumn.getValue ? titleColumn.getValue(item) : (item as any)[titleColumn.key]
      ...
      return {
        ...
        description: config.descriptionColumn 
          ? String((columns.find(c => c.id === config.descriptionColumn)?.getValue 
        ...
        category: config.categoryColumn 
          ? String((columns.find(c => c.id === config.categoryColumn)?.getValue 
              ? columns.find(c => c.id === config.categoryColumn)!.getValue!(item) 
              : (item as any)[config.categoryColumn!]) || '')
          : undefined,
        color: getColorForCategory(config.categoryColumn 
          ? String((columns.find(c => c.id === config.categoryColumn)?.getValue 
              ? columns.find(c => c.id === config.categoryColumn)!.getValue!(item) 
              : (item as any)[config.categoryColumn!]) || '')
          : undefined),
  ...
  const calendarData = useMemo(() => {
    ...
    return data.map((item, index) => {
      const startDateValue = startDateColumn.getValue ? startDateColumn.getValue(item) : (item as any)[startDateColumn.key]
      const titleValue = titleColumn.getValue ? titleColumn.getValue(item) : (item as any)[titleColumn.key]
      ...
      if (config.endDateColumn) {
        ...
        if (endDateColumn) {
          const endDateValue = endDateColumn.getValue ? endDateColumn.getValue(item) : (item as any)[endDateColumn.key]
