import { FormulaContext, ColumnConfig } from './types'
...
export class FormulaEngine<T = any> {
  ...
  private processReferences(formula: string): string {
    ...
    return formula.replace(cellRefRegex, (match, column, row) => {
  ...
  /**
   * Convertit un index numérique en référence de colonne (A, B, AA, etc.)
   */
  private indexToColumnRef(index: number): string {
    let result = ''
    index++
    while (index > 0) {
      index--
  ...
  private evaluateExpression(expression: string): any {
    ...
    processedExpression = processedExpression.replace(
      ...
      (match, startCol, endCol) => {
        ...
        return values.length > 0 ? (values.reduce((sum, val) => sum + val, 0) / values.length).toString() : '0'
      }
    )
    ...
    processedExpression = processedExpression.replace(/ROUND\(([^,]+),([^)]+)\)/g, '(Math.round(($1) * Math.pow(10, $2)) / Math.pow(10, $2))')
    ...
    // Fonctions de texte
    processedExpression = processedExpression.replace(/UPPER\(([^)]+)\)/g, '($1).toString().toUpperCase()')
    processedExpression = processedExpression.replace(/LOWER\(([^)]+)\)/g, '($1).toString().toLowerCase()')
    processedExpression = processedExpression.replace(/LEN\(([^)]+)\)/g, '($1).toString().length')
    processedExpression = processedExpression.replace(/LEFT\(([^,]+),([^)]+)\)/g, '($1).toString().substring(0, $2)')
    processedExpression = processedExpression.replace(/RIGHT\(([^,]+),([^)]+)\)/g, '($1).toString().slice(-($2))')
    processedExpression = processedExpression.replace(/MID\(([^,]+),([^,]+),([^)]+)\)/g, '($1).toString().substring(($2)-1, ($2)-1+($3))')
    ...
    processedExpression = processedExpression.replace(/YEAR\(([^)]+)\)/g, 'new Date($1).getFullYear()')
    processedExpression = processedExpression.replace(/MONTH\(([^)]+)\)/g, '(new Date($1).getMonth() + 1)')
  ...
  private safeEval(expression: string): any {
    try {
      ...
      console.warn('Erreur d\'évaluation de formule:', error, 'Expression:', expression)
      return '#ERROR'
    }
  }
  ...
  private containsMaliciousCode(expression: string): boolean {
    const forbidden = [
      ...
      '__proto__', 'delete', 'while', 'for'
    ]
    
    return forbidden.some(word => 
      new RegExp(`\\b${word}\\b`, 'i').test(expression)
    )
  }
