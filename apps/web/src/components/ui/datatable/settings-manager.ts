...
import { TableSettings, ColumnConfig } from './types'
...
export class SettingsManager {
  ...
  static saveSettings(tableId: string, settings: TableSettings, userId?: string): void {
    try {
      const key = this.getStorageKey(tableId, userId)
      const data = {
        settings,
        timestamp: Date.now(),
        version: '1.0'
  ...
  static loadSettings(tableId: string, userId?: string): TableSettings | null {
    try {
      ...
      if (this.isExpired(data.timestamp)) {
        this.removeSettings(tableId, userId)
        return null
      }
  ...
  static listSavedTables(userId?: string): string[] {
    try {
      ...
      const tables: string[] = []
      const prefix = this.getStoragePrefix(userId)
      ...
      console.error('Erreur lors du listage des tables:', error)
      return []
    }
  }
  ...
  static importSettings(settingsData: Record<string, any>, userId?: string): void {
    try {
      Object.entries(settingsData).forEach(([tableId, data]) => {
        this.saveSettings(tableId, data.settings, userId)
      })
    } catch (error) {
  ...
  private static getStorageKey(tableId: string, userId?: string): string {
    return `${this.getStoragePrefix(userId)}${tableId}`
  }
  ...
  private static isExpired(timestamp: number): boolean {
    const now = Date.now()
    const expiryTime = this.DEFAULT_EXPIRY_DAYS * 24 * 60 * 60 * 1000
  ...
  static validateSettings<T>(
    ...
    columns.forEach((column, index) => {
      ...
      validatedSettings.columns[column.id] = {
        visible: existingSettings?.visible !== undefined ? existingSettings.visible : column.visible !== false,
...
export const usePersistedTableSettings = <T>(
  ...
  const importSettings = React.useCallback((settingsData: Record<string, any>) => {
