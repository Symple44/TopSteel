...
import { HierarchicalDatatableConfig, HierarchicalItem } from './use-hierarchical-reorder'
...
export function useHierarchicalPreferences(
  ...
  const loadPreferences = useCallback(async () => {
    try {
      ...
      if (orderResponse.ok) {
        ...
        console.warn('Impossible de charger l\'ordre hiérarchique')
        setHierarchyOrder([])
      }
  ...
  const savePreferences = useCallback(async (newConfig: HierarchicalDatatableConfig) => {
    try {
      ...
      if (response.ok) {
        setConfig(newConfig)
        console.log('Préférences hiérarchiques sauvegardées avec succès')
      } else {
  ...
  const saveHierarchyOrder = useCallback(async (items: HierarchyOrderItem[]) => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('Ordre hiérarchique sauvegardé avec succès')
      } else {
  ...
  const applyHierarchyOrder = useCallback(<T extends HierarchicalItem>(
    data: T[]
  ): T[] => {
  ...
  const updateHierarchyFromData = useCallback(<T extends HierarchicalItem>(
    data: T[]
  ) => {
...
function generatePath<T extends HierarchicalItem>(
  item: T, 
  allItems: T[]
): string {
  if (!item.parent_id) {
