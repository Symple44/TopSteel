...
import { RangeSelectionManager, CellPosition, RangeSelection } from './range-selection'
...
export function useRangeSelection() {
  ...
  const selectRange = useCallback((start: CellPosition, end: CellPosition, multiSelect: boolean = false) => {
  ...
  const selectColumn = useCallback((column: string, rows: number[], multiSelect: boolean = false) => {
  ...
  }, [selection]) // Dépend de selection pour re-render
