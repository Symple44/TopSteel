...
import { Button, Separator, Input, Label } from '@erp/ui'
import { 
  Bold, 
  Italic, 
  Underline, 
...
const BACKGROUND_COLORS = [
  'transparent', '#FFEBCD', '#E6F3FF', '#F0FFF0', '#FFF0F5', '#F0F8FF', '#FFFACD', '#F5F5DC',
  '#FFE4E1', '#E0FFFF', '#F0FFFF', '#FAFAD2', '#FFE4B5', '#FFEFD5', '#F5FFFA', '#F0F0F0',
  '#FFCCCB', '#ADD8E6', '#90EE90', '#FFB6C1', '#87CEEB', '#DDA0DD', '#98FB98', '#F0E68C'
]
...
export function RichTextEditor({ 
  ...
  const handleInsertLink = useCallback(() => {
    if (!linkData.url || !editorRef.current) {
      console.warn('URL manquante ou éditeur non trouvé')
      return
    }

    console.log('Insertion du lien:', linkData)
    ...
    try {
      ...
      console.log('execCommand insertHTML:', inserted)
      ...
      if (!inserted) {
        ...
        if (selection && selection.rangeCount > 0) {
          ...
          console.log('Lien inséré manuellement')
        } else {
          ...
          console.log('Lien ajouté à la fin')
        }
      }
    ...
    return (
      <div className="border rounded-md">
        ...
        <div className="flex items-center gap-1 p-2 border-b bg-muted/50">
          ...
          <ToolbarButton onClick={() => executeCommand('underline')} icon={Underline} title="Souligné" />
          <Separator orientation="vertical" className="h-4" />
          <ColorPicker colors={TEXT_COLORS} onColorSelect={setTextColor} title="Couleur du texte" icon={Palette} />
        ...
        <div className="bg-white rounded-lg shadow-xl max-w-4xl max-h-[90vh] flex flex-col w-full mx-4">
          ...
          <div className="border rounded-md mb-4">
            <div className="flex items-center gap-1 p-2 border-b bg-muted/50 flex-wrap">
              {/* Formatage de base */}
              <ToolbarButton onClick={() => executeCommand('bold')} icon={Bold} title="Gras (Ctrl+B)" />
              <ToolbarButton onClick={() => executeCommand('italic')} icon={Italic} title="Italique (Ctrl+I)" />
              <ToolbarButton onClick={() => executeCommand('underline')} icon={Underline} title="Souligné (Ctrl+U)" />
              <ToolbarButton onClick={() => executeCommand('strikeThrough')} icon={Strikethrough} title="Barré" />
              
              <Separator orientation="vertical" className="h-4 mx-2" />
              
              {/* Alignement */}
              <ToolbarButton onClick={() => executeCommand('justifyLeft')} icon={AlignLeft} title="Aligner à gauche" />
              <ToolbarButton onClick={() => executeCommand('justifyCenter')} icon={AlignCenter} title="Centrer" />
              <ToolbarButton onClick={() => executeCommand('justifyRight')} icon={AlignRight} title="Aligner à droite" />
              
              <Separator orientation="vertical" className="h-4 mx-2" />
              
              {/* Listes */}
              <ToolbarButton onClick={() => executeCommand('insertUnorderedList')} icon={List} title="Liste à puces" />
              <ToolbarButton onClick={() => executeCommand('insertOrderedList')} icon={ListOrdered} title="Liste numérotée" />
              <ToolbarButton onClick={() => executeCommand('formatBlock', 'blockquote')} icon={Quote} title="Citation" />
              
              <Separator orientation="vertical" className="h-4 mx-2" />
              
              {/* Couleurs */}
              <ColorPicker colors={TEXT_COLORS} onColorSelect={setTextColor} title="Couleur du texte" icon={Palette} />
              <ColorPicker colors={BACKGROUND_COLORS} onColorSelect={setBackgroundColor} title="Couleur de fond" icon={PaintBucket} />
