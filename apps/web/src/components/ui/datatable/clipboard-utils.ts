...
export class ClipboardUtils {
  ...
  static dataToClipboard<T>(data: T[], columns: ColumnConfig<T>[]): string {
    ...
    const rows = data.map(row => {
      return columns.map(col => {
        ...
        return this.formatCellForClipboard(value, col)
      }).join('\t')
    })
  ...
  static validatePastedData<T>(
    ...
    const columnMap = this.mapColumnsToHeaders(columns, headers)
    ...
    dataRows.forEach((row, rowIndex) => {
      ...
      columns.forEach((column, colIndex) => {
        ...
        try {
          const validationResult = this.validateAndConvertValue(rawValue, column)
  ...
  private static validateAndConvertValue<T>(
    ...
    try {
      switch (column.type) {
        ...
        case 'date':
          const date = this.parseDate(trimmedValue)
          if (!date) {
    ...
    try {
      ...
      if (navigator.clipboard && window.isSecureContext) {
        try {
          ...
          // Permission refusée - utiliser la méthode fallback
          console.warn('Permission clipboard refusée, utilisation du fallback')
          return null
        }
      }
      return null
    } catch (error) {
      console.warn('Erreur lors de la lecture du presse-papier:', error)
      return null
    }
  }
