...
function ConnectionLine({ 
  ...
  return (
    <div className="absolute top-0 bottom-0" style={{ left: `${(level - 1) * 24 + 12}px` }}>
      ...
      {/* Ligne horizontale */}
      <div className={cn(
        "absolute top-6 w-3 h-px bg-border transition-colors duration-300",
        "left-0"
      )} />
    </div>
  )
    ...
    <div className="relative">
      ...
      >
        <div className="flex items-center">
          ...
          <div className="flex-1 grid gap-4 p-4" style={{ gridTemplateColumns: `repeat(${columns.filter(c => c.visible !== false).length}, 1fr)` }}>
            {columns.filter(col => col.visible !== false).map(column => (
              <div key={column.id} className="min-w-0">
                <div className={cn(
                  "truncate",
                  displayConfig.compactMode ? "text-sm" : "text-base"
                )}>
...
export function HierarchicalDataTable<T extends HierarchicalItem = HierarchicalItem>({
  ...
  const handleDragOverWithPosition = useCallback((e: React.DragEvent, targetId: string) => {
    e.preventDefault()
    e.stopPropagation()
    ...
    <div className={cn('space-y-4', className)}>
      ...
      <div className="bg-muted/30 border border-border rounded-lg p-4">
        <div className="flex items-center">
          ...
          <div className="flex-1 grid gap-4" style={{ gridTemplateColumns: `repeat(${visibleColumns.length}, 1fr)` }}>
