...
function buildHierarchicalTree<T extends HierarchicalItem>(
  ...
  const buildNode = (item: T, level: number, parent?: HierarchicalTreeNode<T>, path: string = ''): HierarchicalTreeNode<T> => {
  ...
  const buildTreeRecursively = (
    ...
    sortByOrder(items).forEach((item, index) => {
...
function flattenVisibleTree<T extends HierarchicalItem>(
  ...
  const traverse = (nodes: HierarchicalTreeNode<T>[]) => {
    nodes.forEach(node => {
      ...
      const shouldShowParent = !hideEmptyParents || !node.hasChildren || node.children.some(child => 
        showOnlyLevels.length === 0 || showOnlyLevels.includes(child.level)
      )
...
export function useHierarchicalReorder<T extends HierarchicalItem = HierarchicalItem>(
  ...
  const handleDragStart = useCallback((node: HierarchicalTreeNode<T>) => {
  ...
  const updateConfig = useCallback((newConfig: Partial<HierarchicalDatatableConfig>) => {
    const updatedConfig = { ...config, ...newConfig }
    setConfig(updatedConfig)
    onConfigChange?.(updatedConfig)
  }, [config, onConfigChange])

  const updateHierarchyFilters = useCallback((filters: Partial<HierarchyFilters>) => {
    updateConfig({
  ...
  return {
    ...
    hasChildren: (nodeId: string) => flattenedTree.find(n => n.id === nodeId)?.hasChildren || false
  }
}