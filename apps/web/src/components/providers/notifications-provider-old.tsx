...
export function NotificationsProvider({ children }: NotificationsProviderProps) {
  ...
  const connectWebSocket = useCallback(async () => {
    ...
    // Temporairement désactiver Socket.IO et utiliser un polling simple
    console.log('[NotificationsProvider] Mode polling activé - Socket.IO désactivé')
    
    // Simuler une connexion en mode polling
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        console.error('[NotificationsProvider] Erreur polling:', error)
      }
    }, 10000) // Polling toutes les 10 secondes
    
    // Stocker l'interval pour nettoyage
    socketRef.current = { pollInterval, connected: true }
    
    /*
    // Code Socket.IO désactivé temporairement
    try {
      ...
      socket.on('connect', () => {
        console.log('[NotificationsProvider] Socket.IO connecté')
        dispatch({ type: 'SET_CONNECTED', payload: true })
        dispatch({ type: 'SET_ERROR', payload: null })
      ...
      socket.on('disconnect', (reason) => {
        console.log('[NotificationsProvider] Socket.IO déconnecté:', reason)
      ...
      socket.on('connect_error', (error) => {
        console.error('[NotificationsProvider] Erreur Socket.IO:', error)
        dispatch({ type: 'SET_CONNECTED', payload: false })
        dispatch({ type: 'SET_ERROR', payload: 'Erreur de connexion Socket.IO' })
      })
      ...
      socket.on('notification', (notification) => {
        try {
          ...
          console.error('[NotificationsProvider] Erreur traitement notification:', error)
        }
      })
      ...
      console.error('[NotificationsProvider] Erreur création Socket.IO:', error)
      dispatch({ type: 'SET_ERROR', payload: 'Impossible de créer la connexion Socket.IO' })
    }
    */
