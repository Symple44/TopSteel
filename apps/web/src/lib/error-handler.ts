  ...
  static formatError(error: unknown): FormattedError {
    if (error instanceof AxiosError) {
      ...
      if (error.response?.status === 400 && apiError?.errors?.length) {
        return ErrorHandler.formatValidationError(apiError.errors[0])
      }

      // Erreur spécifique avec message du backend
      if (apiError?.message) {
        return ErrorHandler.formatSpecificError(error.response?.status || 500, apiError.message)
      }

      // Erreur HTTP générique
      return ErrorHandler.formatHttpError(error.response?.status || 500)
    }
