...
// Module-level state for ID generation
let idGeneratorCounter = 0
let idGeneratorNodeId: string
const idGeneratorIsClient = typeof window !== 'undefined'
let idGeneratorEnvironment: 'production' | 'development' | 'test'
...
function initializeIdGenerator() {
  ...
  if (idGeneratorIsClient) {
    idGeneratorNodeId = initializeClientNodeId()
  } else {
    idGeneratorNodeId = initializeServerNodeId()
  }
}
...
function generateFallbackNodeId(): string {
  ...
  try {
    ...
    if (idGeneratorIsClient) {
      ...
      try {
        ...
        result = generateFallbackNanoId(length, defaultAlphabet)
      }
    } else {
...
function generateFallbackNanoId(length: number, alphabet: string): string {
  let result = ''
  ...
  try {
    ...
    switch (format) {
      case 'uuid':
        ...
        break
      case 'base58':
        mainId = generateBase58(length)
        break
      case 'hex':
        mainId = generateHex(length)
        break
      case 'base36':
        mainId = generateBase36(length)
        break
      default:
        mainId = generateNanoid(length)
    }
    ...
    if (checksum) {
      const checksumValue = calculateChecksum(result)
...
export function validateId(id: string, expectedPrefix?: keyof typeof ID_PREFIXES): boolean {
  try {
...
export const ID = {
  ...
  uuid: () => generateUuid(),
  nano: (length?: number) => generateNanoid(length),

  // Générateurs métier
  projet: (config?: Partial<IDConfig>) =>
    generateBusinessId('PROJET', { timestamp: true, ...config }),
  devis: (config?: Partial<IDConfig>) =>
    generateBusinessId('DEVIS', { timestamp: true, checksum: true, ...config }),
  facture: (config?: Partial<IDConfig>) =>
    generateBusinessId('FACTURE', { timestamp: true, checksum: true, ...config }),
  client: (config?: Partial<IDConfig>) =>
    generateBusinessId('CLIENT', { format: 'base58', ...config }),
  commande: (config?: Partial<IDConfig>) =>
    generateBusinessId('COMMANDE', { timestamp: true, ...config }),
  stock: (config?: Partial<IDConfig>) => generateBusinessId('STOCK', { format: 'hex', ...config }),
  production: (config?: Partial<IDConfig>) =>
    generateBusinessId('PRODUCTION', { timestamp: true, ...config }),
  user: (config?: Partial<IDConfig>) => generateBusinessId('USER', { format: 'uuid', ...config }),
  session: (config?: Partial<IDConfig>) =>
    generateBusinessId('SESSION', { format: 'uuid', ...config }),
  toast: (config?: Partial<IDConfig>) =>
    generateBusinessId('TOAST', { format: 'nanoid', length: 8, ...config }),

  // Utilitaires
  parse: (id: string) => parseId(id),
  validate: (id: string, prefix?: keyof typeof ID_PREFIXES) => validateId(id, prefix),
  ...
  custom: (prefix: keyof typeof ID_PREFIXES, config?: IDConfig) =>
    generateBusinessId(prefix, config),

  // Legacy (compatibilité)
  generate: () => generateNanoid(),
  simple: () => generateNanoid(8),
}
...
export function useClientId(prefix?: keyof typeof ID_PREFIXES, config?: Partial<IDConfig>): string {
  ...
  useEffect(() => {
    try {
      const newId = prefix ? generateBusinessId(prefix, config) : generateNanoid()
