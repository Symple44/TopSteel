...
export function useGlobalSearch(options: UseGlobalSearchOptions = {}) {
  ...
  const getHistorySuggestions = useCallback((input: string): string[] => {
  ...
  const getEngineStatus = useCallback(async () => {
    try {
      ...
      console.error('Failed to get search engine status:', error)
      return null
    }
  }, [])
  ...
  const getFacetsForType = useCallback((type: string) => {
  ...
  const getResultCountByType = useCallback((type: string): number => {
