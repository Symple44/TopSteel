...
const CACHE_NAME = 'topsteel-marketplace-v1';
  ...
  event.waitUntil(
    caches.keys().then((cacheNames) => {
      return Promise.all(
        cacheNames
          .filter((cacheName) => {
            return cacheName.startsWith('topsteel-') &&
...
async function syncCart() {
  try {
    ...
    const cartRequests = requests.filter(req => 
      req.url.includes('/api/marketplace/cart')
    );
...
async function syncWishlist() {
  try {
    const cache = await caches.open(DYNAMIC_CACHE_NAME);
    const requests = await cache.keys();
    const wishlistRequests = requests.filter(req => 
      req.url.includes('/api/marketplace/wishlist')
    );
