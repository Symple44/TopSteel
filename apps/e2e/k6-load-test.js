...
const testProducts = [
  ...
  { id: 'prod-3', name: 'Welding Machine Pro', price: 1899.00 },
...
export default function (data) {
  ...
  group('Browse Products', () => {
    ...
    const productsRes = http.get(
      `${BASE_URL}/api/marketplace/products?page=1&limit=20`,
    ...
    if (handleResponse(productsRes, 'Get Products')) {
      ...
      const productRes = http.get(
        `${BASE_URL}/api/marketplace/products/${product.id}`,
        { headers: getHeaders(user.token) }
      );
  ...
  group('Shopping Cart Operations', () => {
    ...
    if (handleResponse(addToCartRes, 'Add to Cart')) {
      ...
      if (Math.random() > 0.8) {
        const removeRes = http.delete(
          `${BASE_URL}/api/marketplace/cart/remove/${product.id}`,
          { headers: getHeaders(user.token) }
        );
  ...
  group('User Account', () => {
    ...
    const ordersRes = http.get(
      `${BASE_URL}/api/marketplace/orders/my-orders`,
      { headers: getHeaders(user.token) }
    );
    ...
    const addressesRes = http.get(
      `${BASE_URL}/api/marketplace/addresses`,
      { headers: getHeaders(user.token) }
    );
    ...
    // Get wishlist
    const wishlistRes = http.get(
      `${BASE_URL}/api/marketplace/wishlist`,
      { headers: getHeaders(user.token) }
    );
