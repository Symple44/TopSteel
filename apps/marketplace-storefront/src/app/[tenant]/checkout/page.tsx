...
export default function CheckoutPage({ params }: CheckoutPageProps) {
  ...
  useEffect(() => {
    ...
    if (items.length === 0) {
      router.push(`/${params.tenant}/cart`)
    }
  }, [params.tenant, setTenant, items.length, router])

  const handleSubmitInfo = (e: React.FormEvent) => {
    e.preventDefault()
    
    // Validation
    const requiredFields = ['email', 'firstName', 'lastName', 'phone', 'address', 'city', 'postalCode']
        ...
        <div className="max-w-2xl mx-auto text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground">
              Votre commande a été prise en compte. Vous recevrez un email de confirmation avec les détails de votre commande.
            </p>
          </div>
      ...
      <div className="space-y-8">
        ...
        <div className="flex items-center justify-center space-x-4">
          <div className={cn(
            ...
            checkoutStep === 'info' ? 'bg-primary text-primary-foreground' : 'bg-green-100 text-green-700'
          )}>
          ...
          <div className={cn(
            'w-8 h-px',
            checkoutStep !== 'info' ? 'bg-green-500' : 'bg-muted'
          )} />
          <div className={cn(
            'flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium',
            checkoutStep === 'payment' ? 'bg-primary text-primary-foreground' : 
            checkoutStep === 'confirmation' ? 'bg-green-100 text-green-700' : 'bg-muted text-muted-foreground'
          )}>
                  ...
                  <div className="space-y-3">
                    ...
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input
                        ...
                        onChange={(e) => setCustomerInfo({...customerInfo, newsletter: e.target.checked})}
                        className="rounded border-input"
                      />
                    ...
                    <label className="flex items-start gap-3 cursor-pointer">
                      ...
                      <span className="text-sm">
                        J'accepte les{' '}
                        <Link href={`/${params.tenant}/terms`} className="text-primary hover:underline">
                          conditions générales de vente
                        </Link>{' '}
                        et la{' '}
                        <Link href={`/${params.tenant}/privacy`} className="text-primary hover:underline">
                    ...
                    {paymentInfo.method === 'bank_transfer' && (
                      <div className="p-4 bg-muted/30 rounded-lg">
                        <p className="text-sm text-muted-foreground">
                          ...
                          La commande sera expédiée après réception du paiement.
                        </p>
                      </div>
                    )}
                  </div>
