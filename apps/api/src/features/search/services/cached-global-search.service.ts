...
export class CachedGlobalSearchService implements ISearchStrategy {
  ...
  private extractTenantId(options: SearchOptions): string | null {
    ...
    if (options.filters && options.filters.tenantId) {
  ...
  /**
   * Check if current search should be cached
   * Some searches might be excluded from caching (e.g., admin searches, one-time queries)
   */
  private shouldCacheSearch(options: SearchOptions): boolean {
    // Don't cache very large result sets
    if (options.limit && options.limit > 100) {
      return false
    }

    // Don't cache searches with very specific filters that are unlikely to be repeated
    if (options.filters && Object.keys(options.filters).length > 5) {
      return false
    }
