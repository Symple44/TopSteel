...
import type { PriceCalculationResult, PricingContext } from './pricing-engine.service'
...
export class PricingCacheService {
   ...
   */
  async set(context: PricingContext, result: PriceCalculationResult, ttl?: number): Promise<void> {
    try {
      const key = this.generateCacheKey(context)
