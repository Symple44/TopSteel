...
import { UnitConversionService, UnitType } from '@erp/utils'
...
export class PricingEngineService {
   ...
   */
  async calculatePrice(context: PricingContext, options?: { 
    detailed?: boolean
    ...
    if (article.coefficientVente && article.coefficientVente !== 1) {
      ...
      this.logger.log(`Coefficient de vente appliqué: ${article.coefficientVente} -> Prix ajusté: ${basePrice}€`)
  ...
  private calculateSurfaceBasedPrice(
    ...
    if (!surface && article?.longueur && article?.largeur) {
      surface = UnitConversionService.calculateSurface(
        article.longueur,
        article.largeur,
        'MM'
      )
  ...
  async previewRule(
    ...
    const originalRules = await this.getApplicableRules(
      { id: articleId } as any,
      { ...context, articleId }
    )
    ...
    return {
      ...
      totalDiscountPercentage: basePrice > 0 ? ((basePrice - ruleResult.newPrice) / basePrice) * 100 : 0
    }
  }
}