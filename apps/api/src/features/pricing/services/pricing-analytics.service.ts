...
export class PricingAnalyticsService {
  ...
  async getDashboard(
    ...
    const averageDiscountPercentage = logs.length > 0
      ? logs.reduce((sum, log) => {
          const percentage = log.basePrice > 0 
            ? (log.discount / log.basePrice) * 100 
            : 0
    ...
    const topRules: RuleAnalytics[] = Array.from(ruleStats.entries())
      .map(([ruleId, stats]) => {
        ...
        const conversionRate = stats.usageCount > 0 
          ? (stats.appliedCount / stats.usageCount) * 100 
          : 0
        
        const averageDiscount = stats.appliedCount > 0
          ? stats.totalDiscount / stats.appliedCount
          : 0
    ...
    const averageCalculationTime = logs.length > 0
    ...
    return {
      ...
      conversionFunnel: {
        ...
        rulesApplicable,
        rulesApplied,
        conversionRate: rulesEvaluated > 0 
          ? (rulesApplied / rulesEvaluated) * 100 
          : 0
      },
      performanceMetrics: {
        averageCalculationTime,
  ...
  private generateRecommendations(data: {
    ...
    if (data.averageCalculationTime > 100) {
      recommendations.push('⚡ Temps de calcul élevé détecté. Considérez l\'optimisation des règles complexes.')
    }
    
    if (data.cacheHitRate < 50) {
      recommendations.push('📊 Taux de cache faible. Activez le préchauffage du cache pour les articles populaires.')
    }
    
    if (data.errorRate > 1) {
      recommendations.push('⚠️ Taux d\'erreur élevé. Vérifiez les règles avec formules personnalisées.')
    }
    
    // Business
    if (data.conversionRate < 30) {
      recommendations.push('💡 Taux de conversion faible. Revoyez les conditions des règles pour plus de pertinence.')
    }
    
    // Règles sous-utilisées
    const underusedRules = data.topRules.filter(r => r.usageCount < 10)
    if (underusedRules.length > 0) {
      recommendations.push(`🔍 ${underusedRules.length} règles peu utilisées. Considérez leur archivage ou modification.`)
    }
    
    // Règles très efficaces
    const highPerformers = data.topRules.filter(r => r.effectivenessScore > 80)
    if (highPerformers.length > 0) {
      recommendations.push(`🌟 Règles performantes: ${highPerformers.map(r => r.ruleName).join(', ')}. Dupliquez leur logique pour d'autres segments.`)
    }
  ...
  async exportAnalytics(
    ...
    csv.push('Top Rules')
    csv.push('Rule Name,Usage Count,Conversion Rate,Average Discount,Total Revenue,Effectiveness Score')
    for (const rule of dashboard.topRules) {
      csv.push(`${rule.ruleName},${rule.usageCount},${rule.conversionRate.toFixed(2)}%,${rule.averageDiscount.toFixed(2)},${rule.totalRevenue.toFixed(2)},${rule.effectivenessScore.toFixed(2)}`)
    }
    csv.push('')
    
    csv.push('Performance Metrics')
    csv.push(`Average Calculation Time,${dashboard.performanceMetrics.averageCalculationTime.toFixed(2)}ms`)
