...
export class PricingAnalyticsController {
  ...
  async getDashboard(
    ...
    this.logger.log(
      `Dashboard analytics demandé par ${user.email} pour période ${fromDate.toISOString()} - ${toDate.toISOString()}`
    )
      ...
      return {
        ...
        summary: {
          ...
          averageEffectiveness:
            dashboard.topRules.reduce((sum, rule) => sum + rule.effectivenessScore, 0) /
            dashboard.topRules.length,
