...
import { PricingEngineService, PricingContext, PriceCalculationResult } from '../services/pricing-engine.service'
import { IsString, IsOptional, IsNumber, IsEnum, ValidateNested, IsArray, IsUUID } from 'class-validator'
...
export class PricingController {
  ...
  async calculatePrice(
    ...
    this.logger.log(`Calcul de prix demandé par ${user.email} pour article ${dto.articleId || dto.articleReference}`)
    ...
    try {
      ...
      this.logger.log(`Prix calculé: ${result.basePrice}€ -> ${result.finalPrice}€ (${result.appliedRules.length} règles appliquées)`)
  ...
  @ApiOperation({ 
    ...
    description: 'Applique les règles de prix pour calculer les prix de plusieurs articles en une seule requête'
  })
  ...
  async calculateBulkPrices(
    ...
    this.logger.log(`Calcul de prix en masse demandé par ${user.email} pour ${dto.articles.length} articles`)
    ...
    try {
      const results = await this.pricingEngineService.calculateBulkPrices(
        dto.articles,
        context
      )
  ...
  async previewRule(
    ...
    this.logger.log(`Prévisualisation règle ${dto.ruleId} sur article ${dto.articleId} par ${user.email}`)
    ...
    try {
      const result = await this.pricingEngineService.previewRule(
        dto.ruleId,
        dto.articleId,
        context
      )
