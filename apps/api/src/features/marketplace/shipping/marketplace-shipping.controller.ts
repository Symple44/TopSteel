...
export class MarketplaceShippingController {
  private readonly logger = new Logger(MarketplaceShippingController.name);

  constructor(
    private readonly shippingService: MarketplaceShippingService
  ) {}
  ...
  async getTrackingInfo(@Param('trackingNumber') trackingNumber: string): Promise<MarketplaceShipment | null> {
  ...
  async getCustomerShipments(@Param('customerId') customerId: string): Promise<MarketplaceShipment[]> {
