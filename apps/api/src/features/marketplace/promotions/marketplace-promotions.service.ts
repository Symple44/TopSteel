...
import { Repository, Between, MoreThan, LessThan, In, IsNull, Not } from 'typeorm';
...
export class MarketplacePromotionsService {
   ...
   */
  async createPromotion(tenantId: string, promotionData: CreatePromotionDto): Promise<MarketplacePromotion> {
    try {
      ...
      if (promotionData.requiresCoupon && promotionData.couponCode) {
        await this.createCoupon(tenantId, {
          ...
          type: promotionData.type === 'FREE_SHIPPING' ? 'FREE_SHIPPING' : 
                promotionData.type === 'PERCENTAGE' ? 'PERCENTAGE' : 'FIXED_AMOUNT',
   ...
   */
  async updatePromotion(tenantId: string, updateData: UpdatePromotionDto): Promise<MarketplacePromotion> {
    try {
   ...
   */
  private calculateBuyXGetYDiscount(promotion: MarketplacePromotion, order: MarketplaceOrder): number {
   ...
   */
  private calculateBundleDiscount(promotion: MarketplacePromotion, order: MarketplaceOrder): number {
   ...
   */
  private async getCustomerPromotionUsage(promotionId: string, customerId: string): Promise<number> {
