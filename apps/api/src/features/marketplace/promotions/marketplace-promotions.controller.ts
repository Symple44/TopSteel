...
export class MarketplacePromotionsController {
  private readonly logger = new Logger(MarketplacePromotionsController.name);

  constructor(
    private readonly promotionsService: MarketplacePromotionsService
  ) {}
  ...
  async exportPromotions(
    ...
    const promotions = active 
      ? await this.promotionsService.getActivePromotions(tenantId)
      : []; // Would get all promotions
    ...
    // CSV format
    const csvHeader = [
      'ID', 'Name', 'Type', 'Value', 'Start Date', 'End Date',
      'Active', 'Usage Count', 'Max Usages', 'Minimum Order'
    ].join(',');
