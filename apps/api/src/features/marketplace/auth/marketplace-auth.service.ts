...
export class MarketplaceAuthService {
  ...
  constructor(
    ...
    this.jwtSecret = this.configService.get<string>('MARKETPLACE_JWT_SECRET') || 
                     this.configService.get<string>('JWT_SECRET');
    
    // Security: Ensure JWT secret is always defined
    if (!this.jwtSecret) {
      throw new Error('MARKETPLACE_JWT_SECRET or JWT_SECRET must be defined in environment variables');
    }
    ...
    this.refreshTokenExpiresIn = this.configService.get<string>('MARKETPLACE_REFRESH_TOKEN_EXPIRY') || '7d';
  }
   ...
   */
  async changePassword(customerId: string, data: { currentPassword: string; newPassword: string }): Promise<void> {
    const customer = await this.customerRepository.findOne({
      where: { id: customerId },
  ...
  async resendVerificationEmail(customerId: string): Promise<void> {
    ...
    if (parseInt(attempts || '0') >= 3) {
      throw new BadRequestException('Too many verification emails sent. Please try again in 1 hour.');
    }
  ...
  private async sendWelcomeEmail(customer: MarketplaceCustomer, verificationToken: string): Promise<void> {
    await this.emailService.sendEmail({
      to: customer.email,
      subject: 'Notification Marketplace',
