...
export interface UpdateShopConfigurationDto extends Partial<Omit<ShopConfiguration, 'id' | 'tenantId' | 'createdAt' | 'updatedAt'>> {}
...
export class ShopConfigurationService {
   ...
   */
  async exportConfiguration(tenantId: string): Promise<{ config: ShopConfiguration; exportedAt: Date }> {
    const config = await this.getShopConfiguration(tenantId);
  ...
  private async clearConfigurationCache(tenantId: string): Promise<void> {
    try {
      const patterns = [
        `shop-config:${tenantId}`,
        `config-section:${tenantId}:*`
      ];
