...
import { Between, MoreThan, type Repository } from 'typeorm'
...
export class MarketplaceDashboardService {
  ...
  async getDashboardMetrics(tenantId: string, dateRange?: DateRange): Promise<DashboardMetrics> {
    ...
    try {
      ...
      // Calculate metrics
      const [overview, recentActivity, analytics, performance] = await Promise.all([
  ...
  private async getOverviewMetrics(
    ...
    const [totalRevenue, totalOrders, totalCustomers, totalProducts, paidOrders] =
      await Promise.all([
  ...
  private async getAnalytics(
    tenantId: string,
    dateRange?: DateRange
  ): Promise<DashboardMetrics['analytics']> {
    ...
    const [salesTrend, topProducts, orderStatusBreakdown, customerGrowth] = await Promise.all([
      ...
      this.customerRepository
        ...
        .select(['DATE(customer.created_at) as date', 'COUNT(*) as newCustomers'])
  ...
  private async getPerformanceMetrics(
    ...
    const processingTimes = await this.orderRepository
      .createQueryBuilder('order')
      .select(['EXTRACT(EPOCH FROM (order.shipped_at - order.created_at))/3600 as hours'])
      .where('order.tenant_id = :tenantId', { tenantId })
    ...
    const averageProcessingTime =
      processingTimes.length > 0
        ? processingTimes.reduce((sum, row) => sum + parseFloat(row.hours || '0'), 0) /
    ...
    const deliverySuccessRate =
      totalShipments > 0 ? (successfulDeliveries / totalShipments) * 100 : 0
