...
export { ProductFilters, ProductSortOptions, PaginationOptions, ProductListResponse } from '../adapters/marketplace-product.adapter';
...
export class ProductCatalogService {
   ...
   */
  async createProduct(tenantId: string, productData: CreateMarketplaceProductDto): Promise<MarketplaceProductView> {
    try {
      ...
      if (existingArticle) {
        throw new BadRequestException(`Article with reference ${productData.reference} already exists`);
      }
      ...
      const marketplaceProduct = await this.adapter.getMarketplaceProductById(tenantId, savedArticle.id);
      ...
      this.logger.log(`Marketplace product created: ${savedArticle.designation} (${savedArticle.reference})`);
   ...
   */
  async updateProduct(tenantId: string, updateData: UpdateMarketplaceProductDto): Promise<MarketplaceProductView> {
    try {
      ...
      if (updateData.reference && updateData.reference !== article.reference) {
        ...
        if (existingArticle && existingArticle.id !== article.id) {
          throw new BadRequestException(`Article with reference ${updateData.reference} already exists`);
        }
      }
      ...
      // Convert to marketplace view
      const marketplaceProduct = await this.adapter.getMarketplaceProductById(tenantId, updatedArticle.id);
      ...
      this.logger.log(`Marketplace product updated: ${updatedArticle.designation} (${updatedArticle.reference})`);
   ...
   */
  async bulkUpdateProducts(tenantId: string, bulkUpdate: BulkUpdateDto): Promise<{ updated: number }> {
    try {
  ...
  async restoreProduct(tenantId: string, productId: string): Promise<MarketplaceProductView> {
    try {
      ...
      // Convert to marketplace view
      const marketplaceProduct = await this.adapter.getMarketplaceProductById(tenantId, restoredArticle.id);
   ...
   */
  async getCategories(tenantId: string): Promise<Array<{ category: string; subcategories: string[]; count: number }>> {
    try {
   ...
   */
  async updateProductStock(tenantId: string, productId: string, quantity: number, reason?: string): Promise<void> {
    const article = await this.articleRepository.findOne({
      where: { id: productId }
    });
