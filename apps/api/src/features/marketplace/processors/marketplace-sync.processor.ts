...
export class MarketplaceSyncProcessor {
    ...
    try {
      switch (action) {
        ...
        case 'UPDATE':
          ...
          if (article && article.isMarketplaceEnabled) {
  ...
  async handleStockSync(job: Job<SyncStockJob>) {
    ...
    try {
      ...
      this.logger.error(`Failed to update stock for article ${articleId}: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async handleFullSync(job: Job<FullSyncJob>): Promise<any> {
    ...
    try {
      ...
      this.logger.log(`Full sync completed for tenant ${tenantId}: ${result.synced} synced, ${result.failed} failed`);
  ...
  private async updateProductPrice(productId: string, newPrice: number, tenantId: string): Promise<void> {
  ...
  private async updateProductStock(productId: string, newStock: number, tenantId: string): Promise<void> {
    await this.articleRepository.update(
      { id: productId },
      { 
  ...
  private async updateProductStatus(productId: string, isActive: boolean, tenantId: string): Promise<void> {
    await this.articleRepository.update(
      { id: productId },
