import { Controller, Get, Post, Param, Query, UseGuards, Logger, ValidationPipe, UsePipes } from '@nestjs/common';
import { ApiTags, ApiOperation, ApiResponse, ApiQuery } from '@nestjs/swagger';
import { JwtAuthGuard } from '../../../domains/auth/security/guards/jwt-auth.guard';
import { CurrentTenant } from '../../../core/common/decorators/current-tenant.decorator';
import { ERPMarketplaceIntegrationService, ERPMarketplaceStats, IntegrationHealth, SyncOperationResult } from '../integration/erp-marketplace-integration.service';
import { MarketplaceProductAdapter, ProductListResponse, ProductFilters, ProductSortOptions, PaginationOptions } from '../adapters/marketplace-product.adapter';
import { MarketplaceCustomerAdapter, MarketplaceCustomerView } from '../adapters/marketplace-customer.adapter';
import { MarketplaceOrderAdapter, ERPOrderView, MarketplaceOrderFilters } from '../adapters/marketplace-order.adapter';
import { 
  ProductParamsDto, 
  CustomerParamsDto, 
  OrderParamsDto,
...
export class ERPIntegrationController {
  ...
  async getERPOrdersView(
    @CurrentTenant() tenantId: string,
    @Query() query: OrderFiltersQueryDto
  ) {
