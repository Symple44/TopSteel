...
const ALLOWED_SORT_FIELDS = ['designation', 'prixVenteHT', 'stockDisponible', 'createdAt', 'updatedAt'] as const;
...
export class MarketplaceProductAdapter {
   ...
   */
  async getMarketplaceProductById(tenantId: string, productId: string): Promise<MarketplaceProductView | null> {
   ...
   */
  async getMarketplaceProductBySku(tenantId: string, sku: string): Promise<MarketplaceProductView | null> {
    const article = await this.articleRepository.findOne({
      where: { 
        reference: sku,
        status: ArticleStatus.ACTIF,
   ...
   */
  async toggleMarketplaceEnabled(tenantId: string, productId: string, enabled: boolean): Promise<void> {
   ...
   */
  async getMarketplaceCategories(tenantId: string): Promise<Array<{ category: string; subcategories: string[]; count: number }>> {
  ...
  private articleToMarketplaceView(article: Article, tenantId?: string): MarketplaceProductView {
    ...
    return {
      ...
      dimensions: article.longueur && article.largeur && article.hauteur ? {
   ...
   */
  private applySecureFilters(queryBuilder: SelectQueryBuilder<Article>, filters: ProductFilters): void {
    // Validation et sanitisation des filtres
    if (filters.category && typeof filters.category === 'string' && filters.category.length <= 100) {
      queryBuilder.andWhere('article.famille = :category', { category: filters.category });
    }

    if (filters.subcategory && typeof filters.subcategory === 'string' && filters.subcategory.length <= 100) {
      queryBuilder.andWhere('article.sousFamille = :subcategory', { subcategory: filters.subcategory });
    }
    ...
    if (filters.priceMin !== undefined && typeof filters.priceMin === 'number' && filters.priceMin >= 0) {
      queryBuilder.andWhere('article.prixVenteHT >= :priceMin', { priceMin: filters.priceMin });
    }

    if (filters.priceMax !== undefined && typeof filters.priceMax === 'number' && filters.priceMax >= 0) {
      queryBuilder.andWhere('article.prixVenteHT <= :priceMax', { priceMax: filters.priceMax });
    }

    // Validation des stocks (nombres positifs)
    if (filters.stockMin !== undefined && typeof filters.stockMin === 'number' && filters.stockMin >= 0) {
      queryBuilder.andWhere('article.stockDisponible >= :stockMin', { stockMin: filters.stockMin });
    }

    if (filters.stockMax !== undefined && typeof filters.stockMax === 'number' && filters.stockMax >= 0) {
    ...
    if (filters.tags && Array.isArray(filters.tags) && filters.tags.length > 0 && filters.tags.length <= 10) {
      ...
      if (validTags.length > 0) {
        queryBuilder.andWhere(
          "article.marketplaceSettings->'tags' ?| array[:tags]",
          { tags: validTags }
        );
      }
    }
