...
import { Article, ArticleType, ArticleStatus, UniteStock, MethodeValorisationStock } from '@erp/entities';
...
export class MarketplaceToERPMigrationService {
  ...
  constructor(
    ...
    private readonly marketplaceOrderItemRepository: Repository<MarketplaceOrderItem>,
   ...
   */
  async createMigrationPlan(tenantId: string): Promise<MigrationPlan> {
    ...
    if (existingArticles > 0) {
      warnings.push(`${existingArticles} articles ERP existants - vérification des conflits nécessaire`);
    }

    if (existingPartners > 0) {
      warnings.push(`${existingPartners} partenaires ERP existants - vérification des conflits nécessaire`);
    }
  ...
  async executeMigration(tenantId: string): Promise<MigrationResult> {
    ...
    try {
      ...
      this.logger.log(`Migration completed: ${articlesUpdated} articles, ${customersProcessed} customers, ${ordersUpdated} orders in ${duration}ms`);
