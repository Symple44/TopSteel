    ...
    if (!preferences.length) {
      ...
      const defaultMenus = [
        ...
        { menuId: 'admin', isVisible: true, order: 2, customLabel: 'Administration' },
        {
          ...
          isVisible: true,
          order: 21,
          customLabel: 'Gestion des utilisateurs',
        },
        { menuId: 'admin-roles', isVisible: true, order: 22, customLabel: 'Gestion des rôles' },
        {
          menuId: 'admin-menu-config',
          isVisible: false,
          order: 23,
          customLabel: 'Configuration des menus',
        },
        {
          menuId: 'admin-company',
          isVisible: false,
          order: 24,
          customLabel: 'Informations entreprise',
        },
        { menuId: 'admin-database', isVisible: false, order: 25, customLabel: 'Base de données' },
        {
          menuId: 'admin-notifications-rules',
          isVisible: false,
          order: 26,
          customLabel: 'Règles de notification',
        },
        {
          menuId: 'admin-sessions',
          isVisible: false,
          order: 27,
          customLabel: 'Sessions utilisateurs',
        },
        {
          menuId: 'admin-translations',
          isVisible: false,
          order: 28,
          customLabel: 'Gestion des traductions',
        },
        ...
        { menuId: 'settings', isVisible: true, order: 3, customLabel: 'Paramètres' },
        {
          ...
          isVisible: true,
          order: 31,
          customLabel: 'Personnalisation du menu',
        },
        {
          menuId: 'settings-security',
          isVisible: false,
          order: 32,
          customLabel: 'Paramètres de sécurité',
        },
    ...
    try {
      ...
      try {
        ...
        if (menuData.type === 'custom_menu_data' && Array.isArray(menuData.menuItems)) {
          ...
          this.logger.log(
            `Menu personnalisé récupéré pour l'utilisateur ${userId} avec ${menuItems.length} éléments`
          )
          return menuItems
        }
      } catch (parseError) {
      ...
      this.logger.error(
        `Erreur lors de la récupération du menu personnalisé pour ${userId}:`,
        error
      )
      throw error
    }
