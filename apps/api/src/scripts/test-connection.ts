  ...
  for (const dbName of possibleDatabases) {
    ...
    try {
      ...
      if (importantTables.length > 0) {
        console.log(`   📦 Tables importantes trouvées: ${importantTables.map((t: any) => t.table_name).join(', ')}`);
        
        // Vérifier les sociétés existantes
        try {
          const societes = await dataSource.query('SELECT code, raison_sociale FROM societes LIMIT 5');
      ...
      console.log(`❌ Échec connexion à "${dbName}": ${error instanceof Error ? error.message : String(error)}`);
      console.log('');
    }
  }
