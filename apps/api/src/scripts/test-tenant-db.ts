    ...
    for (const societe of societes) {
      ...
      try {
        ...
        if (tables.find((t: any) => t.table_name === 'articles')) {
          const articleCount = await tenantDataSource.query('SELECT COUNT(*) as count FROM articles');
          console.log(`   📄 Articles: ${articleCount[0].count}`);
        }
        
        if (tables.find((t: any) => t.table_name === 'system_settings')) {
          const settingsCount = await tenantDataSource.query('SELECT COUNT(*) as count FROM system_settings');
