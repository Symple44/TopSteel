...
import { MarketplaceService, CreateModuleDto, UpdateModuleDto, InstallModuleDto, ModuleSearchFilters } from '../services/marketplace.service'
import { MarketplaceModule as MarketplaceModuleEntity, MarketplaceCategory } from '../entities/marketplace-module.entity'
  ...
  async getMarketplaceStats() {
    ...
    const stats = {
      ...
      averageRating: modules.length > 0 
        ? Number((modules.reduce((sum, m) => sum + m.ratingAverage, 0) / modules.length).toFixed(2))
        : 0,
