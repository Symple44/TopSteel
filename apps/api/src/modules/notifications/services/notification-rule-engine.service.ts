...
export class NotificationRuleEngineService {
  ...
  async processEvent(event: NotificationEvent): Promise<void> {
    ...
    try {
      ...
      await this.ruleService.eventRepository.save(event)
      ...
      this.logger.log(`Event processed in ${executionTime}ms. Rules triggered: ${rulesTriggered}, Notifications created: ${notificationsCreated}`)
  ...
  private async processRuleForEvent(rule: NotificationRule, event: NotificationEvent): Promise<boolean> {
    const startTime = Date.now()
    ...
    try {
      ...
      this.logger.log(`Rule ${rule.id} triggered successfully, notification ${notification.id} created`)
      return true
  ...
  private prepareTemplateVariables(eventData: Record<string, any>, rule: NotificationRule): Record<string, any> {
    ...
    switch (rule.trigger.type) {
      case TriggerType.STOCK:
        ...
        variables.threshold_percentage = variables.threshold ? Math.round((variables.quantity / variables.threshold) * 100) : 0
        break
  ...
  private async createNotificationFromRule(rule: NotificationRule, variables: Record<string, any>): Promise<Notifications> {
    ...
    try {
      ...
      const actionUrl = config.actionUrl ? this.ruleService.substituteVariables(config.actionUrl, variables) : undefined

      // Calculer la date d'expiration
      const expiresAt = config.expiresIn ? new Date(Date.now() + config.expiresIn * 60 * 60 * 1000) : undefined
  ...
  async processAllPendingEvents(): Promise<void> {
    ...
    const pendingEvents = await this.ruleService.eventRepository.find({
  ...
  async reprocessFailedEvents(maxAge: number = 24): Promise<void> {
    ...
    const failedEvents = await this.ruleService.eventRepository.find({
      where: { 
  ...
  async cleanupOldEvents(daysToKeep: number = 30): Promise<number> {
    ...
    const result = await this.ruleService.eventRepository
      .createQueryBuilder()
      .delete()
  ...
  async cleanupOldExecutions(daysToKeep: number = 30): Promise<number> {
    ...
    const result = await this.ruleService.executionRepository
      .createQueryBuilder()
      .delete()
  ...
  async triggerEvent(
    ...
    const notificationEvent = await this.ruleService.createEvent(
      type,
      event,
      data,
      source,
  ...
  async testRule(ruleId: string, testData: Record<string, any>): Promise<{
    ...
    try {
      ...
      const message = this.ruleService.substituteVariables(config.messageTemplate, templateVariables)
      const actionUrl = config.actionUrl ? this.ruleService.substituteVariables(config.actionUrl, templateVariables) : undefined
