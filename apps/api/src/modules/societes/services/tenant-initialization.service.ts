...
export class TenantInitializationService {
  ...
  private async createDefaultUsers(societe: Societe): Promise<User[]> {
    ...
    for (const userData of defaultUsers) {
      try {
        ...
        const societeRole = userData.role === UserRoleEnum.ADMIN ? UserSocieteRole.ADMIN : UserSocieteRole.USER
        ...
        this.logger.error(`❌ Erreur création utilisateur ${userData.email}:`, (error as Error).message)
   ...
   */
  private async associateUserToSociete(userId: string, societeId: string, role: UserSocieteRole): Promise<void> {
    try {
      const association = this.societeUserRepository.create({
        userId,
        societeId,
        role: role,
        actif: true,
      ...
      await this.societeUserRepository.save(association)
  ...
  private async initializeSystemParameters(societeCode: string): Promise<void> {
    try {
      ...
      const systemParameters = [
        { key: 'company_name', value: `Société ${societeCode}`, description: 'Nom de la société', category: 'general' },
        { key: 'currency', value: 'EUR', description: 'Devise par défaut', category: 'financial' },
        { key: 'vat_rate', value: '20.0', description: 'Taux de TVA par défaut', category: 'financial' },
        { key: 'decimal_precision', value: '2', description: 'Précision décimale', category: 'general' },
        { key: 'auto_numbering', value: 'true', description: 'Numérotation automatique', category: 'general' },
        { key: 'backup_enabled', value: 'true', description: 'Sauvegardes activées', category: 'system' },
