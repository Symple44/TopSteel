...
export class DatabaseStatsService {
  ...
  async optimizeDatabase(): Promise<{ success: boolean; message: string; details?: any }> {
    try {
      const results: Array<{ table?: string; operation?: string; status: string; message: string }> = []
      ...
      for (const table of tables) {
        try {
          ...
          await this.dataSource.query(`VACUUM ANALYZE ${table.tablename}`)
          results.push({
  ...
  private async getQueryPerformance(): Promise<{ avgResponseTime: number; slowQueries: number }> {
    try {
      ...
      const slowResult = await this.dataSource.query(`
  ...
  private async getTablesSizes(): Promise<Array<{ tableName: string; totalSize: string; rowCount: number; indexSize: string }>> {
    try {
      const result = await this.dataSource.query(`
        SELECT 
          schemaname,
