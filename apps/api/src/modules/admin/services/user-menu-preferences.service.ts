...
export class UserMenuPreferencesService {
  ...
  async resetUserPreferences(userId: string): Promise<UserMenuPreferences> {
    ...
    await this.itemPreferencesRepository.delete({ userPreferencesId: preferences.id })
  ...
  async executeMenuItemAction(userId: string, action: MenuItemAction): Promise<void> {
    ...
    let itemPreference = await this.itemPreferencesRepository.findOne({
      where: {
    ...
    if (!itemPreference) {
      itemPreference = UserMenuItemPreference.create(
        preferences.id,
        action.menuItemId
      )
    }
    ...
    await Promise.all([
      this.itemPreferencesRepository.save(itemPreference),
      this.preferencesRepository.save(preferences)
    ])
  }

  async updateCustomMenuItem(
    userId: string,
    customItemDto: CustomMenuItemDto
  ): Promise<void> {
    const preferences = await this.getUserPreferences(userId)
    ...
    await this.itemPreferencesRepository.save(itemPreference)
  }
    ...
    const processItems = (items: MenuTreeNode[]): UserMenuWithPreferences[] => {
      return items
        ...
        .sort((a, b) => {
          ...
          const aOrder = a.userPreferences?.customOrder ?? preferences.getItemOrder(a.id!) ?? a.orderIndex
          const bOrder = b.userPreferences?.customOrder ?? preferences.getItemOrder(b.id!) ?? b.orderIndex
  ...
  async importUserPreferences(userId: string, importData: any): Promise<UserMenuPreferences> {
    ...
    if (importData.itemPreferences) {
      ...
      await this.itemPreferencesRepository.save(itemPreferences)
    }
