...
export class TracabiliteService {
  ...
  async findOne(id: string): Promise<Tracabilite> {
    this.logger.log(Récupération tracabilite id: .\script.ps1{id});
  ...
  async create(data: Partial<Tracabilite>, userId?: string): Promise<Tracabilite> {
    this.logger.log(Création nouveau tracabilite par user: .\script.ps1{userId});
  ...
  async update(id: string, data: Partial<Tracabilite>, userId?: string): Promise<Tracabilite> {
    this.logger.log(Mise à jour tracabilite id: .\script.ps1{id} par user: .\script.ps1{userId});
  ...
  async remove(id: string, userId?: string): Promise<void> {
    this.logger.log(Suppression logique tracabilite id: .\script.ps1{id} par user: .\script.ps1{userId});
    await this.repository.update(id, { 
      actif: false, 
      updated_by: userId 
    });
