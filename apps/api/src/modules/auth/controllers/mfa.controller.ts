...
export class MFAController {
  ...
  async setupTOTP(@Request() req: any, @Body() body: SetupTOTPDto) {
    try {
      ...
      const result = await this.mfaService.setupTOTP(
        userId,
        userEmail,
        body.phoneNumber
      )
  ...
  async setupWebAuthn(@Request() req: any, @Body() body: SetupWebAuthnDto) {
    try {
      ...
      const result = await this.mfaService.setupWebAuthn(
        userId,
        userEmail,
        body.userName
      )
  ...
  async initiateMFA(@Request() req: any, @Body() body: InitiateMFADto) {
    try {
      ...
      const result = await this.mfaService.initiateMFASession(
        userId,
        body.mfaType,
        req
      )
      ...
      throw new HttpException(
        'Erreur lors de l\'initiation MFA',
        HttpStatus.INTERNAL_SERVER_ERROR
      )
    }
  }
