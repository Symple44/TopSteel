...
export class MFAService {
  ...
  async setupTOTP(
    ...
    try {
      ...
      if (existingTOTP && existingTOTP.isVerified) {
        return {
          success: false,
  ...
  async setupWebAuthn(
    ...
    try {
      ...
      mfaRecord.metadata = {
        ...mfaRecord.metadata,
        ...(({ pendingChallenge: challenge, pendingChallengeExpiry: new Date(Date.now() + 5 * 60 * 1000).toISOString() } as any))
      }
      await this.userMFARepository.save(mfaRecord)
  ...
  async verifyAndAddWebAuthn(
    ...
    try {
      ...
      const pendingChallenge = (mfaRecord.metadata as any)?.pendingChallenge
      const challengeExpiry = (mfaRecord.metadata as any)?.pendingChallengeExpiry

      if (!pendingChallenge || !challengeExpiry) {
        return { success: false, error: 'Challenge WebAuthn manquant' }
      }

      if (new Date() > new Date(challengeExpiry)) {
      ...
      if (mfaRecord.metadata) {
        delete (mfaRecord.metadata as any).pendingChallenge
        delete (mfaRecord.metadata as any).pendingChallengeExpiry
      }
  ...
  async verifyMFA(
    ...
    try {
      ...
      const mfaRecord = await this.userMFARepository.findOne({
        where: { userId: mfaSession.userId, type: mfaSession.mfaType, isEnabled: true, isVerified: true }
      })
      ...
      if (mfaSession.mfaType === 'totp') {
        ...
        if (verification.verified) {
          ...
          if (verification.newCounter !== undefined) {
            mfaRecord.updateWebAuthnCounter(credentialId, verification.newCounter)
            await this.userMFARepository.save(mfaRecord)
          }
        }
      }
   ...
   */
  async disableMFA(userId: string, mfaType: 'totp' | 'webauthn', verificationCode?: string): Promise<{
    success: boolean
    error?: string
  }> {
    try {
  ...
  async getMFAStats(userId: string): Promise<{
    ...
    const stats = {
      ...
      methods: {
        ...
        webauthn: { enabled: false, verified: false, credentialsCount: 0, lastUsed: undefined as Date | undefined },
