...
export class SessionRedisService {
  ...
  async addActiveSession(sessionData: ActiveSessionData, ttlSeconds: number = 24 * 60 * 60): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`Session active ajoutée: ${sessionData.sessionId} pour user ${sessionData.userId}`)
    } catch (error) {
  ...
  async forceLogoutUser(userId: string): Promise<string[]> {
    ...
    try {
      ...
      this.logger.log(`Utilisateur ${userId} forcé à se déconnecter, ${removedSessions.length} sessions supprimées`)
