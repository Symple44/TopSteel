...
export class TOTPService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.encryptionKey = this.configService.get<string>('MFA_ENCRYPTION_KEY') || this.generateDefaultKey()
    
    if (!this.configService.get<string>('MFA_ENCRYPTION_KEY')) {
      this.logger.warn('MFA_ENCRYPTION_KEY non configuré, utilisation d\'une clé par défaut (non sécurisé pour la production)')
    }
  }
   ...
   */
  verifyBackupCode(code: string, encryptedCodes: string): {
    isValid: boolean
