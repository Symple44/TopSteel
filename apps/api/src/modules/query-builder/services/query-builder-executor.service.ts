...
export class QueryBuilderExecutorService {
  ...
  async executeQuery(
    ...
    try {
      // Execute count query
      const countResult = await this.dataSource.query(countQuery, whereParams)
      const total = parseInt(countResult[0].total, 10)

      // Execute main query
      const data = await this.dataSource.query(query, [...whereParams, effectivePageSize, offset])
  ...
  async getAvailableColumns(queryBuilderId: string, userId: string): Promise<any[]> {
