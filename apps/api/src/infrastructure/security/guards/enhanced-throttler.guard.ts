...
export class EnhancedThrottlerGuard extends ThrottlerGuard {
  protected async getTracker(req: Request): Promise<string> {
    ...
    const realIp =
      typeof forwarded === 'string' ? forwarded.split(',')[0] : req.connection.remoteAddress
  ...
  protected async throwThrottlingException(context: ExecutionContext): Promise<void> {
    ...
    const _tracker = await this.getTracker(request)
  ...
  private getClientIP(request: Request): string {
    ...
    return typeof forwarded === 'string'
      ? forwarded.split(',')[0]
      : request.connection?.remoteAddress || request.ip || 'unknown'
  }
}
