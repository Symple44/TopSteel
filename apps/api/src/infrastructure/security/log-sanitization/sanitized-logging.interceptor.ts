...
export class SanitizedLoggingInterceptor implements NestInterceptor {
  ...
  private sanitizeRequestData(request: AuthenticatedRequest): Partial<AuthenticatedRequest> {
    const sanitizedHeaders = this.logSanitizer.sanitizeLogObject(request.headers) as Record<
      string,
      string
    >
    const sanitizedBody = this.logSanitizer.sanitizeLogObject(request.body) as
      | Record<string, unknown>
      | undefined
    const sanitizedQuery = this.logSanitizer.sanitizeLogObject(request.query) as
      | Record<string, unknown>
      | undefined
    const sanitizedCookies = this.logSanitizer.sanitizeLogObject(request.cookies) as
