    ...
    try {
      ...
      if (migrationTableExists[0]?.exists) {
        ...
        executedMigrations = await this.dataSource.query(`
          SELECT name FROM migrations WHERE name IN ('CreateInitialTables1737178800000', 'CreateAllTables1737179000000')
        `)
      }
      ...
      const allTablesMigration = new CreateAllTables1737179000000()
      ...
      await this.dataSource.transaction(async (manager) => {
        ...
        this.logger.log('🔄 Exécution de CreateAllTables...')
        await allTablesMigration.up(manager.queryRunner!)
        
        // Marquer comme exécutée
        await manager.query(`
          INSERT INTO migrations (timestamp, name) 
          VALUES ($1, $2)
        `, [1737179000000, 'CreateAllTables1737179000000'])
      })
  ...
  async checkBaseTables(): Promise<boolean> {
    try {
      const result = await this.dataSource.query(`
        ...
        WHERE table_schema = 'public' 
        AND table_name IN ('users', 'system_parameters', 'clients', 'roles', 'permissions', 'fournisseurs', 'machines', 'commandes')
      `)
      
      return parseInt(result[0].count) >= 8
    } catch (error) {
