...
export class MigrationService {
    ...
    try {
      ...
      if (executedMigrations.length === 0 && allMigrations.length > 0) {
        ...
        if (this.isProduction) {
          const allowProductionMigrations = this.configService.get<boolean>('ALLOW_PRODUCTION_MIGRATIONS', false)
          if (!allowProductionMigrations) {
            this.logger.warn('⚠️  Migrations bloquées en production. Définir ALLOW_PRODUCTION_MIGRATIONS=true pour autoriser')
            return
          }
        }
  ...
  async isDatabaseUpToDate(): Promise<boolean> {
    try {
      const pendingMigrations = await this.dataSource.showMigrations()
      return !pendingMigrations || (Array.isArray(pendingMigrations) && pendingMigrations.length === 0)
    } catch (error) {
