...
export class DatabasePreSyncService {
  ...
  /**
   * Nettoyage spécialisé avant la synchronisation TypeORM
   */
  async cleanBeforeSync(): Promise<void> {
    this.logger.log('🧹 Nettoyage pré-synchronisation...')
    
    try {
      // 1. Supprimer les index TypeORM spécifiques problématiques
      await this.removeKnownProblematicIndexes()
      
      // 2. Supprimer tous les index auto-générés par TypeORM
      ...
      // 4. Vérifier qu'il n'y a plus d'index problématiques
      await this.verifyNoProblematicIndexes()
      
      this.logger.log('✅ Nettoyage pré-synchronisation terminé')
    } catch (error) {
      this.logger.error('❌ Erreur lors du nettoyage pré-synchronisation:', error)
      throw error
    }
  }

  /**
   * Supprime les index spécifiques connus pour être problématiques
   */
  private async removeKnownProblematicIndexes(): Promise<void> {
    const knownProblematicIndexes = [
      'IDX_e4a5a4bcd15ca9eedd81916638', // Index principal problématique
      'IDX_97672ac88f789774dd47f7c8be3', // Autre index problématique
      'UQ_e4a5a4bcd15ca9eedd81916638',  // Contrainte unique associée
    ...
    for (const indexName of knownProblematicIndexes) {
      try {
        ...
        this.logger.warn(`⚠️  Impossible de supprimer l'index ${indexName}:`, error instanceof Error ? error.message : String(error))
      }
    }
  }

  /**
   * Supprime tous les index auto-générés par TypeORM
   */
    ...
    try {
      const autoGeneratedIndexes = await this.dataSource.query(`
        ...
        AND i.indexname NOT IN (
          ...
          WHERE contype IN ('p', 'f') -- Exclure les clés primaires et étrangères
        )
      `)
      
      this.logger.log(`🔍 ${autoGeneratedIndexes.length} index auto-générés trouvés`)
      
      for (const { indexname, tablename } of autoGeneratedIndexes) {
        try {
          await this.dataSource.query(`DROP INDEX IF EXISTS "${indexname}" CASCADE`)
          this.logger.log(`✅ Index auto-généré ${indexname} (table: ${tablename}) supprimé`)
        } catch (error) {
          this.logger.warn(`⚠️  Impossible de supprimer l'index ${indexname}:`, error instanceof Error ? error.message : String(error))
        }
      }
    } catch (error) {
      this.logger.error('Erreur lors de la suppression des index auto-générés:', error)
    }
  }
  ...
  private async cleanOrphanedUniqueConstraints(): Promise<void> {
    try {
      ...
      this.logger.log(`🔍 ${orphanedConstraints.length} contraintes uniques orphelines trouvées`)
      ...
      for (const { conname, table_name } of orphanedConstraints) {
        try {
          ...
          this.logger.warn(`⚠️  Impossible de supprimer la contrainte ${conname}:`, error instanceof Error ? error.message : String(error))
        }
      }
    } catch (error) {
  ...
  /**
   * Vérifie qu'il n'y a plus d'index problématiques
   */
  private async verifyNoProblematicIndexes(): Promise<void> {
    ...
    if (problematicIndexes.length > 0) {
      ...
      this.logger.log('✅ Aucun index problématique détecté')
    }
  }

  /**
   * Supprime une table spécifique si elle existe
   */
  async dropTableIfExists(tableName: string): Promise<void> {
    try {
      ...
      this.logger.warn(`⚠️  Impossible de supprimer la table ${tableName}:`, error instanceof Error ? error.message : String(error))
    }
  }

  /**
   * Supprime toutes les tables liées aux user_menu_preferences
   */
