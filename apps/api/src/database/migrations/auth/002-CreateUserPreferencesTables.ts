...
export class CreateUserPreferencesTables1721808002000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // Créer les index
    await queryRunner.query('CREATE UNIQUE INDEX IDX_user_settings_user_key ON user_settings (user_id, key)')
    await queryRunner.query('CREATE UNIQUE INDEX IDX_datatable_prefs_user_table ON datatable_hierarchical_preferences (user_id, table_name)')
    await queryRunner.query('CREATE UNIQUE INDEX IDX_datatable_order_user_table ON datatable_hierarchy_order (user_id, table_name)')
    await queryRunner.query('CREATE UNIQUE INDEX IDX_ui_prefs_user_component ON ui_preferences_reorderable_list (user_id, component_id)')
    await queryRunner.query('CREATE UNIQUE INDEX IDX_user_menu_prefs_user ON user_menu_preferences (user_id)')
    await queryRunner.query('CREATE UNIQUE INDEX IDX_user_menu_item_prefs ON user_menu_item_preferences (user_id, menu_item_id)')
    await queryRunner.query('CREATE UNIQUE INDEX IDX_user_menu_admin_user ON user_menu_preferences_admin (user_id)')
    ...
    await queryRunner.query('CREATE UNIQUE INDEX IDX_notification_settings_user ON notification_settings (user_id)')
