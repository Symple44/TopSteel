...
export class MaterialMovementService {
  ...
  async createMovement(
    ...
    try {
      ...
      const stockApres = this.calculateNewStock(material.stockPhysique, data.quantite, data.type)
  ...
  async processMovement(
    ...
    if (!options?.skipValidation && this.isSortie(movement.type)) {
      const stockDisponible =
        movement.material.stockPhysique - (movement.material.stockReserve || 0)
   ...
   */
  async findMovements(filters: IMaterialMovementFilters & { tenantId: string }): Promise<{
  ...
  async calculatePeriodBalance(
    ...
    const transformations = movements.filter((m) => this.isTransformation(m.type)).length
  ...
  private isSortie(type: MaterialMovementType): boolean {
    return [MaterialMovementType.SORTIE, MaterialMovementType.PERTE].includes(type)
  }

  private isEntree(type: MaterialMovementType): boolean {
    return [MaterialMovementType.ENTREE, MaterialMovementType.RETOUR].includes(type)
  }
  ...
  private calculateVolume(material: Material, quantite: number): number {
    ...
    if (dims.longueur && dims.largeur) {
      const volume =
        (dims.longueur * dims.largeur * (dims.epaisseur || dims.hauteur || 1)) / 1000000000 // m³
