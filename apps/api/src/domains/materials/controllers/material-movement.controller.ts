...
import { IsEnum, IsNumber, IsOptional, IsString, IsUUID, IsDate, Min, IsBoolean, IsObject, ValidateNested } from 'class-validator';
...
export class MaterialMovementController {
  ...
  @ApiBody({ 
    schema: {
      ...
      properties: {
        ...
        reverseStock: { type: 'boolean', description: 'Inverser le stock si le mouvement était terminé' }
      },
      required: ['motif']
    }
  })
  ...
  async cancelMovement(
    ...
    return await this.materialMovementService.cancelMovement(
      id,
      body.motif,
      { 
  ...
  async getMaterialMovementHistory(
    ...
    return await this.materialMovementService.getMaterialMovementHistory(
      materialId,
      { limit, includeAnnule, dateDebut, dateFin }
    );
  }
