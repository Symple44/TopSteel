...
/**
 * PermissionPrismaService - Phase Auth Migration
 *
 * Service de gestion des permissions utilisant Prisma
 *
 * Fonctionnalités:
 * - Récupération permissions utilisateur (global + société)
 * - Vérification permissions (has/hasAll/hasAny)
 * - Cache Redis pour performances
 * - Support rôles additionnels et restreints
 * - Permission summary et statistiques
 */
@Injectable()
  ...
  async getUserPermissions(userId: string, societeId?: string): Promise<Permission[]> {
    ...
    if (societeId) {
      ...
      if (userSocieteRole) {
        ...
        const societePermissionsData = userSocieteRole.permissions as any
        let additionalPermissions: string[] = []
        let restrictedPermissions: string[] = []

        if (societePermissionsData) {
          if (Array.isArray(societePermissionsData)) {
            additionalPermissions = societePermissionsData
          } else if (typeof societePermissionsData === 'object') {
            additionalPermissions = societePermissionsData.additional || []
            restrictedPermissions = societePermissionsData.restricted || []
          }
        }
