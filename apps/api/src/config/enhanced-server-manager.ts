...
export class EnhancedServerManager {
  ...
  static async cleanupPort(port: number, retries: number = 3): Promise<boolean> {
    ...
    const initialCheck = await this.isPortAvailable(port)
    if (initialCheck) {
      this.logger.debug(`Port ${port} déjà disponible`)
      return true
    }

    for (let attempt = 1; attempt <= retries; attempt++) {
      this.logger.debug(`🔍 Tentative ${attempt}/${retries} - Nettoyage du port ${port}`)
      
      try {
        const killedPids = await this.killProcessOnPort(port)
        
        if (killedPids.length > 0) {
          this.logger.log(`🔧 ${killedPids.length} processus arrêté(s) sur le port ${port}`)
        ...
        if (isAvailable) {
          if (killedPids.length > 0) {
            this.logger.log(`✅ Port ${port} libéré avec succès`)
          }
          return true
        }
        
        if (attempt < retries) {
          this.logger.warn(`⚠️  Port ${port} encore occupé, nouvelle tentative dans 2s...`)
          await this.sleep(2000)
        }
      } catch (error) {
        this.logger.error(`❌ Erreur lors du nettoyage du port ${port}:`, error)
      }
    }
    
    this.logger.error(`❌ Impossible de libérer le port ${port} après ${retries} tentatives`)
    return false
  }
  ...
  static async killProcessOnPort(port: number): Promise<string[]> {
    ...
    try {
          ...
          for (const pid of pids) {
            try {
              ...
              try {
                ...
                this.logger.log(`✅ Processus PID ${pid} forcé à s'arrêter`)
              } catch {
                // Le processus s'est arrêté gracieusement
                this.logger.log(`✅ Processus PID ${pid} arrêté gracieusement`)
              }
              
              killedPids.push(pid)
            } catch (error) {
              this.logger.warn(`⚠️  Processus ${pid} déjà terminé ou inaccessible`)
            }
          }
        } catch (netstatError: any) {
          // netstat ne trouve rien (normal si port libre)
          if (netstatError.code === 1 && netstatError.stdout === '') {
            this.logger.debug(`Port ${port} libre, aucun processus à arrêter`)
            return killedPids
          }
          throw netstatError
        }
      ...
      if (error.code !== 1) {
        this.logger.error(`Erreur lors de la recherche de processus sur le port ${port}:`, error)
      }
    }
  ...
  static async cleanupOrphanedProcesses(): Promise<void> {
    ...
    try {
      if (isWindows) {
        ...
        this.logger.log('Processus Node.js en écoute nettoyés')
      } else {
        // Unix - chercher les processus node orphelins
        await execAsync('pkill -f "node.*nest"')
        this.logger.log('Processus Nest.js orphelins nettoyés')
      }
    } catch (error) {
      // Pas de processus orphelins trouvés
      this.logger.debug('Aucun processus orphelin trouvé')
    }
  }
