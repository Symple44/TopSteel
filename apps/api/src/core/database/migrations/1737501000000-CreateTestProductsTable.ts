...
export class CreateTestProductsTable1737501000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // Create indexes
    await queryRunner.query(
      `CREATE INDEX "IDX_test_products_categoryId" ON "test_products" ("categoryId")`
    )
    await queryRunner.query(`CREATE INDEX "IDX_test_products_sku" ON "test_products" ("sku")`)
    await queryRunner.query(
      `CREATE INDEX "IDX_test_orders_customerId" ON "test_orders" ("customerId")`
    )
    await queryRunner.query(
      `CREATE INDEX "IDX_test_orders_orderDate" ON "test_orders" ("orderDate")`
    )
    await queryRunner.query(
      `CREATE INDEX "IDX_test_order_items_orderId" ON "test_order_items" ("orderId")`
    )
    await queryRunner.query(
      `CREATE INDEX "IDX_test_order_items_productId" ON "test_order_items" ("productId")`
    )
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "test_order_items" DROP CONSTRAINT "FK_test_order_items_productId"`
    )
    await queryRunner.query(
      `ALTER TABLE "test_order_items" DROP CONSTRAINT "FK_test_order_items_orderId"`
    )
    await queryRunner.query(`ALTER TABLE "test_orders" DROP CONSTRAINT "FK_test_orders_customerId"`)
    await queryRunner.query(
      `ALTER TABLE "test_products" DROP CONSTRAINT "FK_test_products_categoryId"`
    )
