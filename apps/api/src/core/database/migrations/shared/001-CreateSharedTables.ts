...
export class CreateSharedTables1737000002000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      'CREATE INDEX IDX_shared_materials_type_forme ON shared_materials (type, forme)'
    )
    ...
    await queryRunner.query(
      'CREATE INDEX IDX_shared_processes_complexite ON shared_processes (complexite)'
    )
    await queryRunner.query(
      'CREATE INDEX IDX_shared_quality_code ON shared_quality_standards (code)'
    )
    await queryRunner.query(
      'CREATE INDEX IDX_shared_quality_type ON shared_quality_standards (type)'
    )

    // Index pour les nouvelles tables
    await queryRunner.query(
      'CREATE UNIQUE INDEX IDX_system_parameters_key ON system_parameters (key)'
    )
    await queryRunner.query(
      'CREATE UNIQUE INDEX IDX_system_settings_module_key ON system_settings (module, key)'
    )
    ...
    await queryRunner.query(
      'CREATE INDEX IDX_menu_item_permissions_menu ON menu_item_permissions (menu_item_id)'
    )
    await queryRunner.query(
      'CREATE INDEX IDX_menu_item_roles_menu ON menu_item_roles (menu_item_id)'
    )
    await queryRunner.query(
    ...
    await queryRunner.query(
      'ALTER TABLE menu_item_permissions ADD CONSTRAINT FK_menu_item_permissions_menu FOREIGN KEY (menu_item_id) REFERENCES menu_items(id) ON DELETE CASCADE'
    )
    await queryRunner.query(
      'ALTER TABLE menu_item_roles ADD CONSTRAINT FK_menu_item_roles_menu FOREIGN KEY (menu_item_id) REFERENCES menu_items(id) ON DELETE CASCADE'
    )
    await queryRunner.query(
      'ALTER TABLE notification_rules ADD CONSTRAINT FK_notification_rules_template FOREIGN KEY (template_id) REFERENCES notification_templates(id) ON DELETE SET NULL'
    )
  }
