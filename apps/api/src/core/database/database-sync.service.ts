...
export class DatabaseSyncService {
  ...
  async safeSynchronize(): Promise<void> {
    ...
    try {
      ...
      if (error instanceof Error && error.message?.includes('existe déjà')) {
        ...
        if (this.isDevelopment) {
          this.logger.warn(
            '🔧 Erreur persistante. Utilisez npm run db:reset pour réinitialiser en développement.'
          )
        }
        throw error
      }
  ...
  private async hasSpecificProblematicIndexes(): Promise<boolean> {
    const problematicIndexes = ['IDX_e4a5a4bcd15ca9eedd81916638', 'IDX_97672ac88f789774dd47f7c8be3']
  ...
  private async closeActiveConnections(): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(
        'Impossible de fermer les connexions:',
    ...
    try {
      ...
      for (const { typname } of enums) {
        try {
          ...
          this.logger.warn(
            `Impossible de supprimer l'enum ${typname}:`,
            error instanceof Error ? error.message : String(error)
          )
        }
      }
    ...
    try {
      ...
      for (const { trigger_name, event_object_table } of triggers) {
        try {
          ...
          this.logger.warn(
            `Impossible de supprimer le trigger ${trigger_name}:`,
            error instanceof Error ? error.message : String(error)
          )
        }
      }
      ...
      for (const { routine_name } of functions) {
        try {
          ...
          this.logger.warn(
            `Impossible de supprimer la fonction ${routine_name}:`,
            error instanceof Error ? error.message : String(error)
          )
        }
      }
    ...
    try {
      ...
      for (const { sequence_name } of sequences) {
        try {
          ...
          this.logger.warn(
            `Impossible de supprimer la séquence ${sequence_name}:`,
            error instanceof Error ? error.message : String(error)
          )
        }
      }
