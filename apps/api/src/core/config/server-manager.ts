...
export class ServerManager {
  ...
  static async killProcessOnPort(port: number): Promise<void> {
    ...
    try {
        ...
        try {
          ...
          for (const pid of pids) {
            try {
              ...
              ServerManager.logger.log(`✅ Processus PID ${pid} arrêté sur le port ${port}`)
            } catch (_error) {
              ServerManager.logger.warn(`⚠️  Impossible de tuer le processus ${pid}`)
            }
          }
        } catch (_error) {
          // Aucun processus trouvé sur ce port
          ServerManager.logger.debug(`Aucun processus trouvé sur le port ${port}`)
        }
      } else {
      ...
      ServerManager.logger.error(
        `Erreur lors de la tentative de libération du port ${port}:`,
        error
      )
    }
  }
