...
import { DatabaseHealthService } from '../modules/database-core/services/database-health.service'
...
export class SystemHealthService {
  ...
  constructor(
    @Optional() private databaseHealthService?: DatabaseHealthService
  ) {}

  async checkSystemHealth(): Promise<SystemHealth> {
    try {
      if (this.databaseHealthService) {
        const dbHealth = await this.databaseHealthService.checkSystemHealth()
        return {
