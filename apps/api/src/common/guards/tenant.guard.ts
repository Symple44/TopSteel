...
export class TenantGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const isCommonDatabase = this.reflector.get<boolean>(
      'commonDatabase',
      context.getHandler(),
    )
    if (isCommonDatabase) {
    ...
    const requiresTenant = this.reflector.get<boolean>(
      'requiresTenant',
      context.getHandler(),
    )
    if (!requiresTenant) {
    ...
    const societeUser = await this.societeUserRepository.findOne({
      where: {
        userId: user.id,
        societeId: societeId,
        actif: true,
    ...
    await this.societeUserRepository.update(societeUser.id, {
