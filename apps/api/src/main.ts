...
async function bootstrap() {
  ...
  if (env !== 'production') {
    ...
    const configV1 = new DocumentBuilder()
      ...
      .addServer(`${process.env.API_URL || `http://127.0.0.1:${port}`}/`, 'Serveur de développement')
      .addServer(`${process.env.API_URL || `http://127.0.0.1:${port}`}/v1`, 'API V1 explicite')
      .build()
    ...
    const configV2 = new DocumentBuilder()
      ...
      .addServer(`${process.env.API_URL || `http://127.0.0.1:${port}`}/v2`, 'API V2 (Bêta)')
      .build()
