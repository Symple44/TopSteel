...
const SENSITIVE_VARIABLES = [
  ...
  'PRIVATE', 'SALT', 'HASH', 'AUTH', 'API_KEY'
];
...
class EnvHarmonizer {
  ...
  generateDefaults() {
    ...
    this.allVariables.forEach((sources, key) => {
      ...
      else if (newKey.startsWith('AUTH_') || newKey.includes('JWT') || newKey.includes('SESSION')) category = 'AUTH';
      else if (newKey.startsWith('SERVICE_') || newKey.includes('_PORT')) category = 'SERVICE';
      else if (newKey.startsWith('SECURITY_') || newKey.includes('CORS') || newKey.includes('RATE_LIMIT')) category = 'SECURITY';
      ...
      else if (['NODE_ENV', 'APP_NAME', 'APP_VERSION', 'LOG_LEVEL'].includes(newKey)) category = 'ENVIRONMENT';
